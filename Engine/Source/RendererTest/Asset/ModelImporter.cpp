...
namespace Cue::Editor
{
    namespace
    {
            ...
            const aiMaterial& material, uint32_t materialIndex) noexcept
        {
            ...
            if (material.Get(AI_MATKEY_NAME, name) == AI_SUCCESS &&
                name.length > 0 && name.C_Str() != nullptr)
            {
            ...
            const aiMaterial& material, uint32_t materialIndex) noexcept
        {
            ...
            {
                materialData.color =
                    Math::float4(diffuseColor.r, diffuseColor.g, diffuseColor.b,
                                 diffuseColor.a);
            }
            if (aiGetMaterialColor(&material, AI_MATKEY_BASE_COLOR,
                                   &diffuseColor) == AI_SUCCESS)
            {
                materialData.color =
                    Math::float4(diffuseColor.r, diffuseColor.g, diffuseColor.b,
            ...
            {
                textureResult = material.GetTexture(aiTextureType_BASE_COLOR, 0,
                                                    &texturePath);
            }
            if (textureResult == AI_SUCCESS &&
            ...
            const aiMatrix4x4& transform) noexcept
        {
            ...
            return Math::make_affine_matrix(
                ...
                Math::Quaternion(rotation.x, -rotation.y, -rotation.z,
                                 rotation.w),
        ...
        [[nodiscard]] Math::float3 convert_scale(
            const aiVector3D& value) noexcept
        {
            return Math::float3(value.x, value.y, value.z);
        }
            ...
            const Core::Native::MeshData& sourceMesh)
        {
            ...
            const size_t uniqueVertexCount = meshopt_generateVertexRemap(
                remap.data(), sourceMesh.indices.data(),
                sourceMesh.indices.size(), sourceVertices.data(),
            ...
            meshopt_remapVertexBuffer(
                remappedVertices.data(), sourceVertices.data(),
                sourceVertices.size(), sizeof(MeshoptVertex), remap.data());
            ...
            const Core::Native::MeshData& sourceMesh)
        {
            ...
            meshopt_optimizeOverdraw(
                overdrawIndices.data(), vertexCacheIndices.data(),
            ...
            const size_t fetchedVertexCount = meshopt_optimizeVertexFetch(
                fetchVertices.data(), overdrawIndices.data(),
                overdrawIndices.size(), sourceVertices.data(),
                sourceVertices.size(), sizeof(MeshoptVertex));
            ...
            size_t& outUniqueVertexCount)
        {
            ...
            const size_t uniqueVertexCount = meshopt_generateVertexRemap(
                remap.data(), sourceMesh.indices.data(),
                sourceMesh.indices.size(), positions.data(), positions.size(),
            ...
            result.normals.assign(uniqueVertexCount,
            ...
            const Core::Native::MeshData& baseMesh)
        {
            ...
            Math::float3 minPosition(baseMesh.positions[0].x,
                                     baseMesh.positions[0].y,
        ...
        [[nodiscard]] Core::Native::MeshData generate_lod_mesh(
            const Core::Native::MeshData& baseMesh, float indexRatio,
            uint32_t lodIndex, LodGenerationStats& outStats)
        {
            ...
            const size_t simplifiedIndexCount = meshopt_simplify(
                ...
                weldedMesh.positions.size(), sizeof(Math::float4),
                targetIndexCount, lod_target_error(lodIndex), 0, &lodError);
            ...
            if (finalIndexCount > targetIndexCount + (targetIndexCount / 5u))
            {
                ...
                const size_t sloppyIndexCount = meshopt_simplifySloppy(
                    ...
                    weldedMesh.indices.size(), &weldedMesh.positions[0].x,
                    weldedMesh.positions.size(), sizeof(Math::float4),
                    targetIndexCount, 1.0f, &sloppyError);
                if (sloppyIndexCount >= 3u &&
                    sloppyIndexCount < finalIndexCount)
                {
                                 ...
                                 const LodGenerationStats* stats)
        {
            ...
            const double remainingRatio =
                baseIndexCount > 0 ? static_cast<double>(resultIndexCount) /
            ...
            Core::Native::ModelData& outModelData)
        {
                 ...
                 ++childIndex)
            {
                ...
                append_render_parts_from_node(
                    scene, *node.mChildren[childIndex], worldTransform,
                    jointIndices, outModelData);
            }
        }
    } // namespace
        ...
        Core::Native::ModelData& outModelData) noexcept
    {
        ...
        Core::IO::Path modelPath{};
        Result result = Core::IO::make_executable_relative_path(
            fileSystem, filePath, modelPath);
        if (!result)
        {
            return result;
        }

        std::vector<std::byte> modelBytes{};
        result = fileSystem.read_all(modelPath, &modelBytes);
        if (!result)
        {
            return result;
        }

        std::string formatHint = modelPath.extension();
        ...
        for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex)
        {
            ...
            for (uint32_t fi = 0; fi < mesh->mNumFaces; ++fi)
            {
                ...
                // aiProcess_Triangulate を使っているので常に face.mNumIndices
                // == 3
                 ...
                 ++lodArrayIndex)
            {
                ...
                Core::Native::MeshData lodMesh = generate_lod_mesh(
                    remappedMesh, indexRatio, lodIndex, lodStats);
            ...
            size_t occluderTargetIndexCount = static_cast<size_t>(
                static_cast<double>(baseIndexCount) *
        ...
        if (outModelData.renderParts.empty())
        {
            ...
            for (uint32_t sourceMeshIndex = 0;
                 sourceMeshIndex < scene->mNumMeshes; ++sourceMeshIndex)
            {
                ...
                renderPart.lodMeshIndices =
                    sourceMeshLodIndices[sourceMeshIndex];
