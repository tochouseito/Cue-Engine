...
namespace Cue::Renderer
{
    namespace
    {
        class FileSystemAssimpStream final : public Assimp::IOStream
        {
          public:
            explicit FileSystemAssimpStream(std::vector<std::byte> data) noexcept
            ...
            size_t Read(void* buffer, size_t size, size_t count) override
            {
                ...
                const size_t readableBytes =
                    std::min(requestedBytes, m_data.size() - m_position);
                std::memcpy(buffer, m_data.data() + m_position, readableBytes);
                m_position += readableBytes;
            ...
            aiReturn Seek(size_t offset, aiOrigin origin) override
            {
                ...
                if (offset > m_data.size() || base > m_data.size() - offset)
                {
                    return aiReturn_FAILURE;
                }

                m_position = base + offset;
        ...
        class FileSystemAssimpIOSystem final : public Assimp::IOSystem
        {
          public:
            FileSystemAssimpIOSystem(Core::IO::IFileSystem& fileSystem,
                                     Core::IO::Path baseDirectory) noexcept
                : m_fileSystem(fileSystem), m_baseDirectory(std::move(baseDirectory))
            {
            }
            ...
            {
                bool exists = false;
                return m_fileSystem.exists(resolve_path(file), &exists) && exists;
            }
            ...
            Assimp::IOStream* Open(const char* file, const char* mode = "rb") override
            {
                ...
                if (!m_fileSystem.read_all(resolve_path(file), &data))
                {
                    return nullptr;
                }

                return new FileSystemAssimpStream(std::move(data));
            }
          ...
          private:
            [[nodiscard]] Core::IO::Path resolve_path(const char* file) const noexcept
            {
                if (file == nullptr)
                {
                    return {};
                ...
                std::string pathText(file);
                std::replace(pathText.begin(), pathText.end(), '\\', '/');
                const Core::IO::Path path(std::move(pathText));
                if (path.is_absolute())
                {
                    return path.normalize();
                }

                return Core::IO::Path::join(m_baseDirectory, path);
            }

            Core::IO::IFileSystem& m_fileSystem;
            Core::IO::Path m_baseDirectory{};
        };
        ...
        make_texture_asset_name(const aiString& texturePath) noexcept
        {
        ...
        [[nodiscard]] bool
        is_embedded_texture_path(const aiString& texturePath) noexcept
        {
            return texturePath.length > 0 && texturePath.C_Str() != nullptr &&
                   texturePath.C_Str()[0] == '*';
        }

        [[nodiscard]] Core::Native::ImportedMaterialData
        import_material(const aiMaterial& material, uint32_t materialIndex) noexcept
        {
        ...
        [[nodiscard]] Math::float4x4
        convert_node_transform(const aiMatrix4x4& transform) noexcept
        {
            aiVector3D scale{};
        ...
        [[nodiscard]] Math::float4x4
        convert_inverse_bind_matrix(const aiMatrix4x4& offsetMatrix) noexcept
        {
        ...
        convert_translation(const aiVector3D& value) noexcept
        {
        ...
        convert_rotation(const aiQuaternion& value) noexcept
        {
        ...
        [[nodiscard]] std::string
        make_render_part_name(const aiNode& node,
        ...
        [[nodiscard]] std::vector<MeshoptVertex>
        pack_vertices(const Core::Native::MeshData& meshData)
        {
        ...
        remap_mesh(const Core::Native::MeshData& sourceMesh)
        {
        ...
        [[nodiscard]] Core::Native::MeshData
        optimize_mesh_for_render(const Core::Native::MeshData& sourceMesh)
        {
            Core::Native::MeshData result = sourceMesh;
            if (sourceMesh.positions.empty() || sourceMesh.indices.empty())
            {
            ...
            meshopt_optimizeVertexCache(vertexCacheIndices.data(),
            ...
            meshopt_optimizeOverdraw(overdrawIndices.data(), vertexCacheIndices.data(),
                                     ...
                                     sourceVertices.size(), sizeof(MeshoptVertex), 1.05f);
        ...
        [[nodiscard]] Core::Native::MeshData
        optimize_mesh(const Core::Native::MeshData& sourceMesh)
        {
        ...
        [[nodiscard]] Core::Native::MeshData
        make_position_welded_mesh(const Core::Native::MeshData& sourceMesh,
        ...
        [[nodiscard]] Core::Native::MeshData
        make_billboard_lod_mesh(const Core::Native::MeshData& baseMesh)
        {
        ...
        [[nodiscard]] Core::Native::MeshData
        generate_lod_mesh(const Core::Native::MeshData& baseMesh, float indexRatio,
                          uint32_t lodIndex, LodGenerationStats& outStats)
        {
            ...
            if (finalIndexCount > targetIndexCount + (targetIndexCount / 5u))
            {
                ...
                const size_t sloppyIndexCount = meshopt_simplifySloppy(
                    simplified.data(), weldedMesh.indices.data(), weldedMesh.indices.size(),
                    &weldedMesh.positions[0].x, weldedMesh.positions.size(),
                    sizeof(Math::float4), targetIndexCount, 1.0f, &sloppyError);
            ...
            Core::Native::ModelData& outModelData)
        {
                 ...
                 ++nodeMeshIndex)
            {
                ...
                if (meshIndex >= outModelData.meshes.size() ||
                    meshIndex >= scene.mNumMeshes || scene.mMeshes[meshIndex] == nullptr)
                {
                    continue;
                }
            ...
            for (uint32_t childIndex = 0; childIndex < node.mNumChildren; ++childIndex)
            {
                ...
                append_render_parts_from_node(scene, *node.mChildren[childIndex],
                                              worldTransform, jointIndices, outModelData);
            }
        }
    } // namespace

    Result
    ModelImporter::import_model(Core::IO::IFileSystem& fileSystem,
                                const Core::IO::Path& filePath,
                                std::string_view modelName,
                                Core::Native::ModelData& outModelData) noexcept
    {
        return import_model(fileSystem, filePath, modelName, LodGroupSettings{},
                            outModelData);
    }

    Result ModelImporter::import_model(
        Core::IO::IFileSystem& fileSystem, const Core::IO::Path& filePath,
        std::string_view modelName, const LodGroupSettings& lodGroupSettings,
        Core::Native::ModelData& outModelData) noexcept
    {
        outModelData = {};
        ...
        importer.SetIOHandler(
            new FileSystemAssimpIOSystem(fileSystem, filePath.parent()));
        ...
        if (scene->mNumMaterials > 0 && scene->mMaterials != nullptr)
        {
                 ...
                 ++materialIndex)
            {
                ...
                outModelData.materials.push_back(
                    import_material(*scene->mMaterials[materialIndex], materialIndex));
            }
        }
        ...
        for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex)
        {
            ...
            log_mesh_lod_result(outModelData.meshes[baseMeshIndex].name, 0u,
                                baseIndexCount, baseIndexCount, baseIndexCount, true,
                                nullptr);
                 ...
                 ++lodArrayIndex)
            {
                ...
                const float indexRatio = lodGroupSettings.indexRatios[lodArrayIndex];
                ...
                log_mesh_lod_result(outModelData.meshes[baseMeshIndex].name, lodIndex,
                                    baseIndexCount, targetIndexCount,
            ...
            size_t occluderTargetIndexCount =
                static_cast<size_t>(static_cast<double>(baseIndexCount) *
                                    static_cast<double>(effectiveOccluderIndexRatio));
            occluderTargetIndexCount = (std::max<size_t>)(3u, occluderTargetIndexCount);
            ...
            if (occluderAccepted)
            {
                occluderMesh.name = outModelData.meshes[baseMeshIndex].name + "_occluder";
                Core::Native::MeshData optimizedOccluderMesh =
                    optimize_mesh_for_render(occluderMesh);
                occluderMeshIndex = static_cast<uint32_t>(outModelData.meshes.size());
            ...
            if (lodGroupSettings.generateBillboardLod)
            {
                ...
                if (!billboardMesh.positions.empty() && !billboardMesh.indices.empty())
                {
                    log_mesh_lod_result(outModelData.meshes[baseMeshIndex].name, 4u,
                                        baseIndexCount, billboardMesh.indices.size(),
                                        billboardMesh.indices.size(), true, nullptr);
                    const uint32_t billboardMeshIndex =
                        static_cast<uint32_t>(outModelData.meshes.size());
                    sourceMeshLodIndices[meshIndex].push_back(billboardMeshIndex);
                    outModelData.meshes.push_back(optimize_mesh_for_render(billboardMesh));
                }
            }
        }
        ...
        if (outModelData.renderParts.empty())
        {
                 ...
                 ++sourceMeshIndex)
            {
                ...
                const uint32_t baseMeshIndex = sourceMeshLodIndices[sourceMeshIndex][0];
                Core::Native::ModelRenderPartData renderPart{};
