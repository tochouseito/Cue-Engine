...
convert_input_element_desc(const InputElementDesc &desc) {
...
D3D12_BLEND_DESC convert_blend_mode(const std::vector<BlendMode> &modes) {
  ...
  for (auto &rtDesc : desc.RenderTarget) {
    // 色書き込みマスクを既定で有効化し、BlendMode::None
    // でも出力が消えないようにする
  ...
  for (size_t i = 0;
       i < modes.size() && i < D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT; ++i) {
...
convert_rasterizer_state(const RasterizerStateDesc &desc) {
...
convert_depth_stencil_state(const DepthStencilStateDesc &desc) {
  D3D12_DEPTH_STENCIL_DESC d3dDesc{};
  d3dDesc.DepthEnable = desc.depthEnable ? TRUE : FALSE;
  d3dDesc.DepthWriteMask = (desc.depthWriteMask == DepthWriteMask::All)
...
convert_descriptor_range_type(RootParameterType type) {
  switch (type) {
...
[[nodiscard]] bool
is_descriptor_table_parameter(RootParameterType type) noexcept {
  switch (type) {
  case RootParameterType::DescriptorTableCBV:
  case RootParameterType::DescriptorTableSRV:
  case RootParameterType::DescriptorTableUAV:
...
[[nodiscard]] D3D12_ROOT_PARAMETER_TYPE
convert_root_parameter_type(RootParameterType type) noexcept {
  switch (type) {
...
Result DX12PipelineManager::create_graphics_pipeline(
  ...
  D3D12_RASTERIZER_DESC rasterizerDesc =
      convert_rasterizer_state(desc.rasterizerState);
  D3D12_DEPTH_STENCIL_DESC depthStencilDesc =
      convert_depth_stencil_state(desc.depthStencilState);

  // ルートシグネチャをRegistryから取得する
  RootSignatureRecord *rootSignatureRecord =
      m_rootSignatureRegistry.ref_get(desc.rootSignatureHandle);
  ...
  psoDesc.PrimitiveTopologyType =
      convert_primitive_topology_type(desc.primitiveTopologyType);
  psoDesc.NumRenderTargets = static_cast<UINT>(desc.rtvFormats.size());
  for (size_t i = 0;
       i < desc.rtvFormats.size() && i < D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT;
       ++i) {
...
Result
DX12PipelineManager::destroy_graphics_pipeline(PipelineStateHandle handle) {
  // ハンドルが有効かをチェックする
  if (!handle.valid()) {
  ...
  // 名前マップから削除する
  for (auto it = m_pipelineNameMap.begin(); it != m_pipelineNameMap.end();
       ++it) {
    if (it->second == handle) {
...
Result DX12PipelineManager::get_graphics_pipeline(std::string_view name,
...
Result DX12PipelineManager::create_compute_pipeline(
    const ComputePipelineStateDesc &desc, PipelineStateHandle &out) {
...
Result
DX12PipelineManager::destroy_compute_pipeline(PipelineStateHandle handle) {
  if (!handle.valid()) {
    return Result::fail(Code::InvalidArgument, Severity::Error,
                        "Invalid compute pipeline state handle.");
  }

  for (auto it = m_computePipelineNameMap.begin();
       it != m_computePipelineNameMap.end(); ++it) {
    if (it->second == handle) {
...
Result DX12PipelineManager::get_compute_pipeline(std::string_view name,
                                                 PipelineStateHandle &out) {
...
Result DX12PipelineManager::create_root_signature(const RootSignatureDesc &desc,
  ...
  // RootParameterDesc を descriptor table / constants に変換し、FrameGraph の
  // descriptor 解決結果をそのまま bind できる形にする
  ...
  for (const RootParameterDesc &parmDesc : desc.parameters) {
    ...
    if (is_descriptor_table_parameter(parmDesc.type)) {
      ...
      descriptorRange.NumDescriptors =
          parmDesc.descriptorCount == 0 ? UINT_MAX : parmDesc.descriptorCount;
...
Result DX12PipelineManager::destroy_root_signature(RootSignatureHandle handle) {
  ...
  // 名前マップから削除する
  for (auto it = m_rootSignatureNameMap.begin();
       it != m_rootSignatureNameMap.end(); ++it) {
    if (it->second == handle) {
...
Result DX12PipelineManager::get_root_signature(std::string_view name,
                                               RootSignatureHandle &out) {
...
Result DX12PipelineManager::create_shader_blob(const ShaderCompileDesc &desc,
                                               ShaderBlobHandle &out) {
  // HLSLCompiler の失敗を Result で受け、失敗時に無効ハンドルを成功扱いしない
  ShaderBlobRecord blobRecord{};
  Result result =
      m_hlslCompiler.compile_shader_raw(desc, &blobRecord.shaderBlob);
  if (!result) {
    return result;
  }
...
Result DX12PipelineManager::destroy_shader_blob(ShaderBlobHandle handle) {
  ...
  // 名前マップから削除する
  for (auto it = m_shaderBlobNameMap.begin(); it != m_shaderBlobNameMap.end();
       ++it) {
    if (it->second == handle) {
...
Result DX12PipelineManager::get_shader_blob(std::string_view name,
                                            ShaderBlobHandle &out) {
...
bool DX12PipelineManager::try_get_graphics_pipeline(
    PipelineStateHandle handle, DX12GraphicsPipelineRecord **outRecord) {
  // ハンドルの解決とレコードの取得
  *outRecord = nullptr;
  *outRecord = m_pipelineRegistry.ref_get(handle);
  return *outRecord != nullptr;
}
bool DX12PipelineManager::try_get_compute_pipeline(
    PipelineStateHandle handle, DX12ComputePipelineRecord **outRecord) {
  *outRecord = nullptr;
  *outRecord = m_computePipelineRegistry.ref_get(handle);
  return *outRecord != nullptr;
}
bool DX12PipelineManager::try_get_root_signature(
    RootSignatureHandle handle, RootSignatureRecord **outRecord) {
  // ハンドルの解決とルートシグネチャの取得
  *outRecord = nullptr;
  *outRecord = m_rootSignatureRegistry.ref_get(handle);
  return *outRecord != nullptr;
}
bool DX12PipelineManager::try_get_shader_blob(ShaderBlobHandle handle,
                                              ShaderBlobRecord **outRecord) {
  // ハンドルの解決とシェーダーブロブの取得
  *outRecord = nullptr;
