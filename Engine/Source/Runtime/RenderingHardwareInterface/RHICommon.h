...
public:
  ...
  virtual Result read_timestamp(uint32_t queryIndex,
                                uint64_t &outValue) const = 0;
  virtual void set_pending_fence(IQueueContext *a_queue,
                                 uint64_t a_fenceValue) = 0;
  ...
  // --- Commaonds ---
  virtual Result resource_barrier(BufferHandle handle,
                                  const ResourceBarrierDesc desc) = 0;
  virtual Result resource_barrier(TextureHandle handle,
                                  const ResourceBarrierDesc desc) = 0;
  virtual Result uav_barrier(BufferHandle handle) = 0;
  virtual Result copy_buffer_region(const BufferCopyRegion &region) = 0;
  virtual Result
  copy_buffer_region_to_readback(const BufferToReadbackCopyRegion &region) = 0;
  virtual Result
  copy_texture_region_to_buffer(const TextureToBufferCopyRegion &region) = 0;
  virtual Result clear_render_target(ViewHandle handle,
                                     const float clearColor[4]) = 0;
  virtual Result clear_depth_stencil(ViewHandle handle, float depth,
                                     uint8_t stencil) = 0;
  virtual Result clear_unordered_access_uint(ViewHandle handle,
                                             const uint32_t clearValues[4]) = 0;
  virtual Result set_viewport_scissor(uint32_t width, uint32_t height) = 0;
  virtual Result set_viewport_scissor(uint32_t x, uint32_t y, uint32_t width,
                                      uint32_t height) = 0;
  ...
  virtual Result set_32bit_constant(uint32_t rootParameterIndex,
                                    uint32_t value) = 0;
  ...
  virtual Result set_graphics_descriptor_table(uint32_t rootParameterIndex,
                                               ViewHandle handle) = 0;
  virtual Result set_compute_descriptor_table(uint32_t rootParameterIndex,
                                              ViewHandle handle) = 0;
  ...
  virtual Result draw_instanced(uint32_t vertexCountPerInstance,
                                uint32_t instanceCount,
                                uint32_t startVertexLocation,
                                uint32_t startInstanceLocation) = 0;
  virtual Result draw_indexed_instanced(uint32_t indexCountPerInstance,
                                        uint32_t instanceCount,
                                        uint32_t startIndexLocation,
                                        int32_t baseVertexLocation,
                                        uint32_t startInstanceLocation) = 0;
  virtual Result execute_instanced_indirect(BufferHandle commandBufferHandle,
                                            uint32_t maxCommandCount) = 0;
  virtual Result execute_indexed_indirect(BufferHandle commandBufferHandle,
...
using commandContextLease =
    std::unique_ptr<ICommandContext, std::function<void(ICommandContext *)>>;
using queueContextLease =
    std::unique_ptr<IQueueContext, std::function<void(IQueueContext *)>>;
...
public:
  ...
  virtual Result get_command_context(CommandListType type,
                                     commandContextLease &outContext) = 0;
...
public:
  ...
  virtual Result get_queue_context(CommandListType type,
                                   queueContextLease &outContext) = 0;
