...
/// @details RHI command を D3D12 API 呼び出しへ変換し、frame ring の slice
/// 解決、
...
public:
  ...
  void set_pending_fence(IQueueContext *a_queue,
                         uint64_t a_fenceValue) override;
  bool is_pending_fence_complete() const override;
  Result wait_for_pending_fence() override;

  // --- 取得 ---
  ID3D12GraphicsCommandList *d3d12_command_list() const {
    return m_commandList.Get();
  }
  ID3D12CommandAllocator *d3d12_command_allocator() const {
  ...
  // --- Commands ---
  Result resource_barrier(BufferHandle handle,
                          const ResourceBarrierDesc desc) override;
  Result resource_barrier(TextureHandle handle,
                          const ResourceBarrierDesc desc) override;
  Result uav_barrier(BufferHandle handle) override;
  Result copy_buffer_region(const BufferCopyRegion &region) override;
  Result copy_buffer_region_to_readback(
      const BufferToReadbackCopyRegion &region) override;
  Result copy_texture_region_to_buffer(
      const TextureToBufferCopyRegion &region) override;
  Result clear_render_target(ViewHandle handle,
                             const float clearColor[4]) override;
  Result clear_depth_stencil(ViewHandle handle, float depth,
                             uint8_t stencil) override;
  Result clear_unordered_access_uint(ViewHandle handle,
                                     const uint32_t clearValues[4]) override;
  Result set_viewport_scissor(uint32_t width, uint32_t height) override;
  Result set_viewport_scissor(uint32_t x, uint32_t y, uint32_t width,
  ...
  Result set_32bit_constant(uint32_t rootParameterIndex,
  ...
  Result set_graphics_descriptor_table(uint32_t rootParameterIndex,
                                       ViewHandle handle) override;
  Result set_compute_descriptor_table(uint32_t rootParameterIndex,
                                      ViewHandle handle) override;
  ...
  Result draw_instanced(uint32_t vertexCountPerInstance, uint32_t instanceCount,
                        uint32_t startVertexLocation,
                        uint32_t startInstanceLocation) override;
  Result draw_indexed_instanced(uint32_t indexCountPerInstance,
                                uint32_t instanceCount,
                                uint32_t startIndexLocation,
                                int32_t baseVertexLocation,
                                uint32_t startInstanceLocation) override;
  Result execute_instanced_indirect(BufferHandle commandBufferHandle,
                                    uint32_t maxCommandCount) override;
  Result execute_indexed_indirect(BufferHandle commandBufferHandle,
                                  BufferHandle commandCountBufferHandle,
                                  uint32_t maxCommandCount) override;

private:
  Result create_command_allocator(ID3D12Device &device,
                                  D3D12_COMMAND_LIST_TYPE type);
  ...
  Result create_timestamp_resources(ID3D12Device &device,
                                    D3D12_COMMAND_LIST_TYPE type);
  Result resolve_slice_index(size_t sliceCount, uint32_t &outIndex) const;
  Result resolve_root_descriptor_buffer(BufferHandle handle,
                                        DX12GpuResource **outResource) const;
  Result resolve_upload_buffer(BufferHandle handle, uint32_t resourceIndex,
                               DX12GpuResource **outResource) const;
...
public:
  DX12CommandPool(DX12RenderDevice &renderDevice,
      ...
      : m_renderDevice(renderDevice),
        ...
        m_graphicsContextPool(
            32, [](DX12GpuCommandContext &ctx) { (void)ctx; },
            [d3d12Device = renderDevice.get_d3d12_device(),
             &descriptorAllocator, &bufferManager, &textureManager,
             &viewManager, &pipelineManager]() {
              return std::make_unique<DX12GpuCommandContext>(
                  *d3d12Device, descriptorAllocator, bufferManager,
                  textureManager, viewManager, pipelineManager,
  ...
  Result get_command_context(CommandListType type,
                             commandContextLease &outContext) override;

  /// @brief submit 後の context を即時再利用せず、fence 完了まで pending
  /// 側に退避する。
...
private:
  ...
  DX12PipelineManager &m_pipelineManager; // パイプラインマネージャへの参照
  Core::Pool<DX12GpuCommandContext,
             std::function<void(DX12GpuCommandContext &)>>
      m_graphicsContextPool;
...
private:
  ...
  Result create_command_queue(ID3D12Device &device,
                              D3D12_COMMAND_LIST_TYPE type);
...
public:
  DX12QueuePool(DX12RenderDevice &renderDevice)
      : m_renderDevice(renderDevice),
        m_graphicsQueuePool(
            1, [](DX12GpuCommandQueue &ctx) { (void)ctx; },
            [d3d12Device = renderDevice.get_d3d12_device()]() {
              return std::make_unique<DX12GpuCommandQueue>(
                  *d3d12Device, D3D12_COMMAND_LIST_TYPE_DIRECT);
            }),
        m_computeQueuePool(
            4, [](DX12GpuCommandQueue &ctx) { (void)ctx; },
            [d3d12Device = renderDevice.get_d3d12_device()]() {
              return std::make_unique<DX12GpuCommandQueue>(
                  *d3d12Device, D3D12_COMMAND_LIST_TYPE_COMPUTE);
            }),
        m_copyQueuePool(
            4, [](DX12GpuCommandQueue &ctx) { (void)ctx; },
            [d3d12Device = renderDevice.get_d3d12_device()]() {
              return std::make_unique<DX12GpuCommandQueue>(
                  *d3d12Device, D3D12_COMMAND_LIST_TYPE_COPY);
            }) {
    // present 用のグラフィックスキューを作成しておき
    m_presentGraphicsQueue = std::make_unique<DX12GpuCommandQueue>(
        *renderDevice.get_d3d12_device(), D3D12_COMMAND_LIST_TYPE_DIRECT);
  }
  ~DX12QueuePool() override = default;

  /// @brief graphics/compute/copy queue を用途別 pool から借りる。
  Result get_queue_context(CommandListType type,
                           queueContextLease &outContext) override;

  /// @brief queue は長寿命なので、返却時は pool へ戻すだけで GPU
  /// 同期待ちは行わない。
...
private:
  DX12RenderDevice &m_renderDevice;
  Core::Pool<DX12GpuCommandQueue, std::function<void(DX12GpuCommandQueue &)>>
      m_graphicsQueuePool;
  std::mutex m_graphicsPoolMutex;
  Core::Pool<DX12GpuCommandQueue, std::function<void(DX12GpuCommandQueue &)>>
      m_computeQueuePool;
