...
namespace Cue::RHI::DX12
{
    ...
    void D3D12Backend::set_win_platform(
        PAL::Win::WinPlatform* a_platform) noexcept
    {
        m_platform = a_platform;
        m_hlslCompiler->set_file_system(
            m_platform != nullptr ? &m_platform->file_system() : nullptr);
    }

    Result D3D12Backend::initialize(const RenderBackendSetupInfo& a_info)
    {
        // 引数の保存
        // ウィンドウサイズやバッファ数は SwapChain/FrameController
        // と共有される基本設定なので、
        ...
        // レンダーデバイスの初期化
        // DXGI factory と ID3D12Device は後続の manager
        // がすべて依存するため最初に作る。
        ...
        // デスクリプタアロケータの初期化
        // View/Texture/Buffer manager が使う CPU/GPU descriptor heap
        // をまとめて確保する。
        ...
        m_textureManager = std::make_unique<DX12TextureManager>(
            *m_renderDevice, *m_descriptorAllocator);
        m_viewManager = std::make_unique<DX12ViewManager>(
            *m_bufferManager, *m_textureManager, *m_descriptorAllocator);
        m_pipelineManager =
            std::make_unique<DX12PipelineManager>(*m_renderDevice, *m_hlslCompiler);

        // コマンドプールの初期化
        m_commandPool = std::make_unique<DX12CommandPool>(
            *m_renderDevice, *m_descriptorAllocator, *m_bufferManager,
            *m_textureManager, *m_viewManager, *m_pipelineManager);

        // コマンドキュープールの初期化
        m_queuePool = std::make_unique<DX12QueuePool>(*m_renderDevice);

        // スワップチェインの初期化
        m_swapChain = std::make_unique<SwapChain>(*m_renderDevice, *m_textureManager,
                                                  *m_viewManager);
        queueContextPtr presentQueueContext =
            m_queuePool->get_present_queue_context();
        if (!presentQueueContext)
        {
            return Result::fail(Code::GetFailed, Severity::Fatal,
                                "Failed to get present graphics queue context for swap "
                                "chain initialization.");
        }
        // スワップチェインの作成にはウィンドウハンドルと present
        // 用グラフィックスキューが必要。
        m_swapChain->create(m_platform->get_window_handle(), a_info.width,
                            a_info.height, a_info.bufferCount,
    ...
    Result D3D12Backend::render(uint64_t a_frameNo, uint32_t a_index,
                                FrameGraph& a_frameGraph)
    {
        a_frameNo;
        return a_frameGraph.execute(a_index);
    }

    Result D3D12Backend::present(uint64_t a_frameNo, uint32_t a_index, bool vsync,
                                 FrameGraph& a_frameGraph)
    {
        a_frameNo;
        a_index;
    ...
    Result
    D3D12Backend::create_frame_graph(const FrameGraphDesc& a_desc,
                                     std::unique_ptr<FrameGraph>& a_outFrameGraph)
    {
