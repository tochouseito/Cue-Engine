...
namespace Cue::RHI::DX12
{
    ...
    Result D3D12Backend::initialize(const RenderBackendSetupInfo& a_info)
    {
        if (m_platform == nullptr)
        {
            return Result::fail(
                ...
                "WinPlatform must be set before D3D12Backend initialization.");
        }
        ...
        m_hlslCompiler =
            std::make_unique<HLSLCompiler>(m_platform->file_system());
        ...
        m_textureManager = std::make_unique<DX12TextureManager>(
            *m_renderDevice, *m_descriptorAllocator, m_platform->file_system());
        m_viewManager = std::make_unique<DX12ViewManager>(
            *m_bufferManager, *m_textureManager, *m_descriptorAllocator);
        m_pipelineManager = std::make_unique<DX12PipelineManager>(
            *m_renderDevice, *m_hlslCompiler);
        ...
        // 用グラフィックスキューが必要。
        m_swapChain->create(
            m_platform->get_window_handle(), a_info.width, a_info.height,
            a_info.bufferCount,
    ...
    Result D3D12Backend::present(uint64_t a_frameNo, uint32_t a_index,
                                 bool vsync, FrameGraph& a_frameGraph)
    {
        a_frameNo;
        a_index;
        Result result =
    ...
    Result D3D12Backend::create_frame_graph(
