...
struct IndirectDispatchCommand final {
...
[[nodiscard]] bool supports_draw_indexed_indirect_signature(
  ...
  // ExecuteIndirect で draw object index を root constant
  // として渡す前提を満たすか確認する。
...
[[nodiscard]] Result
validate_root_binding_command_type(CommandListType type,
                                   const char *bindTarget) {
  // Copy queue では root signature / descriptor bind
  // が無効なので、呼び出し入口で止める。
  if (type == CommandListType::Copy) {
    return Result::fail(Code::InvalidArgument, Severity::Error, bindTarget);
  }
...
[[nodiscard]] D3D12_RESOURCE_STATES normalize_resource_state_for_command_list(
    CommandListType commandListType, D3D12_RESOURCE_STATES state) noexcept {
  // Queue 種別ごとに許可される resource state が違うため、barrier
  // 発行前に安全側へ正規化する。
...
[[nodiscard]] std::array<wchar_t, k_eventNameCapacity>
make_pix_event_name(const char *name) noexcept {
...
DX12GpuCommandContext::DX12GpuCommandContext(
    ID3D12Device &device, DescriptorAllocator &descriptorAllocator,
    DX12BufferManager &bufferManager, DX12TextureManager &textureManager,
    DX12ViewManager &viewManager, DX12PipelineManager &pipelineManager,
    D3D12_COMMAND_LIST_TYPE type)
    : m_descriptorAllocator(descriptorAllocator),
      m_bufferManager(bufferManager), m_textureManager(textureManager),
      m_viewManager(viewManager), m_pipelineManager(pipelineManager),
      m_device(&device) {
  // コマンドアロケータの作成
...
Result DX12GpuCommandContext::reset() {
  // D3D12 の allocator は関連 command list の実行完了後でなければ reset
  // できない。 その完了管理は CommandPool の pending fence 側で担保する。
...
CommandListType DX12GpuCommandContext::type() const { return m_type; }
...
Result DX12GpuCommandContext::write_timestamp(uint32_t queryIndex) {
  ...
  m_commandList->EndQuery(m_timestampQueryHeap.Get(),
                          D3D12_QUERY_TYPE_TIMESTAMP, queryIndex);
  return Result::ok();
}
Result DX12GpuCommandContext::resolve_timestamps(uint32_t firstQueryIndex,
                                                 uint32_t queryCount) {
  if (!supports_timestamps()) {
    return Result::ok();
  }
  ...
  m_commandList->ResolveQueryData(
      m_timestampQueryHeap.Get(), D3D12_QUERY_TYPE_TIMESTAMP, firstQueryIndex,
      queryCount, m_timestampReadbackBuffer.Get(),
      static_cast<UINT64>(firstQueryIndex) * sizeof(uint64_t));
  return Result::ok();
}
Result DX12GpuCommandContext::read_timestamp(uint32_t queryIndex,
...
void DX12GpuCommandContext::set_pending_fence(IQueueContext *a_queue,
                                              uint64_t a_fenceValue) {
  // submit 後に context を pool へ戻す前、対応する queue fence
  // を保存して再利用可否を判定する。
...
void DX12GpuCommandContext::end_event() {
  ...
  // begin_event で積んだスコープを閉じ、GPU/Timing capture
  // 上のパス範囲を確定する
  PIXEndEvent(m_commandList.Get());
  PIXEndEvent();
}
Result DX12GpuCommandContext::resource_barrier(BufferHandle handle,
                                               const ResourceBarrierDesc desc) {
  // ハンドルからリソースを取得する
  ...
  d3d12Barrier.Transition.StateBefore =
      normalize_resource_state_for_command_list(
          type(), resource->get_current_d3d12_state());
  d3d12Barrier.Transition.StateAfter =
      normalize_resource_state_for_command_list(
          type(), convert_resource_state(desc.after));
  d3d12Barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;

  if (d3d12Barrier.Transition.StateBefore ==
      d3d12Barrier.Transition.StateAfter) {
...
Result DX12GpuCommandContext::resource_barrier(TextureHandle handle,
                                               const ResourceBarrierDesc desc) {
  // ハンドルからリソースを取得する
...
Result DX12GpuCommandContext::uav_barrier(BufferHandle handle) {
  DX12BufferRecord *record = nullptr;
  if (!m_bufferManager.try_get_record(handle, &record)) {
    return Result::fail(
        Code::NotFound, Severity::Error,
        "Buffer record was not found for the given UAV barrier handle.");
  }
  ...
  if (!result) {
    return Result::fail(
        result.code, Severity::Error,
        "Failed to resolve buffer slice index for UAV barrier.");
  }
  ...
  if (d3dResource == nullptr) {
    return Result::fail(
        Code::NotFound, Severity::Error,
        "D3D12 resource was not found for the given UAV barrier handle.");
  }

  D3D12_RESOURCE_BARRIER d3d12Barrier{};
  d3d12Barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
  d3d12Barrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
  d3d12Barrier.UAV.pResource = d3dResource;
  m_commandList->ResourceBarrier(1, &d3d12Barrier);
  return Result::ok();
}
Result
DX12GpuCommandContext::copy_buffer_region(const BufferCopyRegion &region) {
  ...
  Result result = resolve_upload_buffer(
      region.srcBufferHandle, region.srcUploadResourceIndex, &srcResource);
  if (!result) {
    return result;
  }

  DX12GpuResource *dstResource = nullptr;
  result = resolve_default_buffer(region.dstBufferHandle,
                                  region.dstDefaultResourceIndex, &dstResource);
  if (!result) {
    return result;
  }
  ...
  m_commandList->CopyBufferRegion(
      dstResource->get_resource(), region.dstByteOffset,
      srcResource->get_resource(), region.srcByteOffset, region.byteSize);
...
Result DX12GpuCommandContext::copy_buffer_region_to_readback(
  ...
  Result result = resolve_default_buffer(
      region.srcBufferHandle, region.srcDefaultResourceIndex, &srcResource);
  if (!result) {
    return result;
  }
  ...
  result = resolve_readback_buffer(
      region.dstBufferHandle, region.dstReadbackResourceIndex, &dstResource);
  if (!result) {
    return result;
  }
...
Result DX12GpuCommandContext::copy_texture_region_to_buffer(
  ...
  Result result = resolve_slice_index(textureRecord->defaultResources.size(),
  ...
  const D3D12_RESOURCE_DESC &textureDesc = textureResource.get_resource_desc();
  ...
  const uint32_t rowPitch = static_cast<uint32_t>(Math::round_up_to_multiple(
      static_cast<uint64_t>(region.width) * texelSize,
      static_cast<uint64_t>(k_textureDataPitchAlignment)));
  const uint64_t requiredBytes =
      region.dstByteOffset + static_cast<uint64_t>(rowPitch) * region.height;
  ...
  m_commandList->CopyTextureRegion(&dstLocation, 0, 0, 0, &srcLocation,
                                   &srcBox);

  return Result::ok();
}
Result DX12GpuCommandContext::clear_render_target(ViewHandle handle,
  ...
  Result result =
      resolve_slice_index(record->defaultTableIds.size(), descriptorIndex);
  if (!result) {
    return Result::fail(result.code, Severity::Error,
                        "Failed to resolve render target descriptor index for "
                        "the current frame.");
  }
  auto cpuHandle = m_descriptorAllocator.get_cpu_handle(
      record->defaultTableIds[descriptorIndex]);
...
Result DX12GpuCommandContext::clear_depth_stencil(ViewHandle handle,
                                                  float depth,
                                                  uint8_t stencil) {
  // ハンドルからビューを取得する
  DX12ViewRecord *record = nullptr;
  ...
  if (!result) {
    return Result::fail(result.code, Severity::Error,
                        "Failed to resolve depth stencil descriptor index for "
                        "the current frame.");
  }
  auto cpuHandle = m_descriptorAllocator.get_cpu_handle(
      record->defaultTableIds[descriptorIndex]);
  ...
  m_commandList->ClearDepthStencilView(
      cpuHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, depth,
      stencil, 0, nullptr);

  return Result::ok();
}
Result DX12GpuCommandContext::clear_unordered_access_uint(
    ViewHandle handle, const uint32_t clearValues[4]) {
  ...
  if (!m_bufferManager.try_get_record(viewRecord->desc.bufferHandle,
                                      &bufferRecord) ||
  ...
  if (!viewRecord->defaultTableIds.empty() &&
      !bufferRecord->defaultResources.empty()) {
    uint32_t descriptorIndex = 0;
    result = resolve_slice_index(viewRecord->defaultTableIds.size(),
                                 descriptorIndex);
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve default UAV descriptor index for "
                          "the current frame.");
    }

    uint32_t resourceIndex = 0;
    result = resolve_slice_index(bufferRecord->defaultResources.size(),
                                 resourceIndex);
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve default UAV buffer slice index "
    ...
    const D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle =
        m_descriptorAllocator.get_cpu_handle(
            viewRecord->defaultTableIds[descriptorIndex]);
    m_commandList->ClearUnorderedAccessViewUint(
        gpuHandle, cpuHandle, resource.get_resource(),
        const_cast<uint32_t *>(clearValues), 0, nullptr);
    return Result::ok();
  }

  if (!viewRecord->uploadTableIds.empty() &&
      !bufferRecord->uploadResources.empty()) {
    uint32_t descriptorIndex = 0;
    result =
        resolve_slice_index(viewRecord->uploadTableIds.size(), descriptorIndex);
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve upload UAV descriptor index for "
                          "the current frame.");
    }

    uint32_t resourceIndex = 0;
    result = resolve_slice_index(bufferRecord->uploadResources.size(),
                                 resourceIndex);
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve upload UAV buffer slice index for "
                          "the current frame.");
    }
    ...
    const D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle =
        m_descriptorAllocator.get_gpu_handle(
            viewRecord->uploadTableIds[descriptorIndex]);
    const D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle =
        m_descriptorAllocator.get_cpu_handle(
            viewRecord->uploadTableIds[descriptorIndex]);
    m_commandList->ClearUnorderedAccessViewUint(
...
Result DX12GpuCommandContext::set_viewport_scissor(uint32_t width,
                                                   uint32_t height) {
  return set_viewport_scissor(0, 0, width, height);
}

Result DX12GpuCommandContext::set_viewport_scissor(uint32_t x, uint32_t y,
                                                   uint32_t width,
                                                   uint32_t height) {
...
Result
DX12GpuCommandContext::set_primitive_topology(PrimitiveTopologyType topology) {
...
Result DX12GpuCommandContext::set_vertex_buffer(uint32_t slot,
                                                BufferHandle handle) {
  if (type() != CommandListType::Graphics) {
    return Result::fail(
        Code::InvalidArgument, Severity::Error,
  ...
  uint32_t resourceIndex = 0;
  Result result =
      resolve_slice_index(bufferRecord->defaultResources.size(), resourceIndex);
  if (!result) {
    return Result::fail(
        result.code, Severity::Error,
  ...
  vertexBufferView.SizeInBytes = static_cast<UINT>(resource.get_buffer_size());
  vertexBufferView.StrideInBytes = bufferRecord->desc.stride;
  m_commandList->IASetVertexBuffers(slot, 1, &vertexBufferView);
  return Result::ok();
}
Result DX12GpuCommandContext::set_index_buffer(BufferHandle handle,
                                               IndexFormat format) {
  if (type() != CommandListType::Graphics) {
    return Result::fail(
        Code::InvalidArgument, Severity::Error,
  ...
  indexBufferView.SizeInBytes = static_cast<UINT>(resource.get_buffer_size());
  indexBufferView.Format = (format == IndexFormat::UInt16)
...
Result
DX12GpuCommandContext::set_graphics_pipeline(PipelineStateHandle handle) {
  if (type() != CommandListType::Graphics) {
    return Result::fail(
        Code::InvalidArgument, Severity::Error,
  ...
  if (!m_pipelineManager.try_get_root_signature(
          pipelineRecord->desc.rootSignatureHandle, &rootSignatureRecord)) {
    return Result::fail(
        Code::NotFound, Severity::Error,
...
Result DX12GpuCommandContext::set_compute_pipeline(PipelineStateHandle handle) {
  Result result = validate_root_binding_command_type(
      type(), "Compute pipeline can only be set on graphics or compute command "
              "contexts.");
  if (!result) {
    return result;
  }
...
Result DX12GpuCommandContext::set_32bit_constant(uint32_t rootParameterIndex,
                                                 uint32_t value) {
  Result result = validate_root_binding_command_type(
      type(), "32-bit constants can only be bound on graphics or compute "
              "command contexts.");
  if (!result) {
    return result;
  }
...
Result DX12GpuCommandContext::set_cbv(uint32_t rootParameterIndex,
                                      BufferHandle handle) {
  Result result = validate_root_binding_command_type(
      type(), "CBV can only be bound on graphics or compute command contexts.");
  if (!result) {
    return result;
  }
  ...
  if (type() == CommandListType::Graphics) {
    ...
    m_commandList->SetComputeRootConstantBufferView(rootParameterIndex,
                                                    gpuAddress);
  }

  return Result::ok();
}
Result DX12GpuCommandContext::set_srv(uint32_t rootParameterIndex,
                                      BufferHandle handle) {
  Result result = validate_root_binding_command_type(
      type(), "SRV can only be bound on graphics or compute command contexts.");
  if (!result) {
    return result;
  }
  ...
  if (type() == CommandListType::Graphics) {
    ...
    m_commandList->SetComputeRootShaderResourceView(rootParameterIndex,
                                                    gpuAddress);
  }

  return Result::ok();
}
Result DX12GpuCommandContext::set_uav(uint32_t rootParameterIndex,
                                      BufferHandle handle) {
  Result result = validate_root_binding_command_type(
      type(), "UAV can only be bound on graphics or compute command contexts.");
  if (!result) {
    return result;
  }
  ...
  if (type() == CommandListType::Graphics) {
    ...
    m_commandList->SetComputeRootUnorderedAccessView(rootParameterIndex,
                                                     gpuAddress);
  }

  return Result::ok();
}
Result DX12GpuCommandContext::set_graphics_descriptor_table(
    uint32_t rootParameterIndex, ViewHandle handle) {
  if (type() != CommandListType::Graphics) {
    return Result::fail(Code::InvalidArgument, Severity::Error,
                        "Graphics descriptor tables can only be set on "
  ...
  D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = m_descriptorAllocator.get_gpu_handle(
      viewRecord->defaultTableIds[descriptorIndex]);
  m_commandList->SetGraphicsRootDescriptorTable(rootParameterIndex, gpuHandle);
  return Result::ok();
}
Result
DX12GpuCommandContext::set_compute_descriptor_table(uint32_t rootParameterIndex,
                                                    ViewHandle handle) {
...
Result
DX12GpuCommandContext::set_graphics_texture_table(uint32_t rootParameterIndex) {
  if (type() != CommandListType::Graphics) {
    return Result::fail(
        Code::InvalidArgument, Severity::Error,
...
Result DX12GpuCommandContext::dispatch(uint32_t groupCountX,
...
Result DX12GpuCommandContext::execute_dispatch_indirect(
  ...
  if (m_device == nullptr) {
    return Result::fail(
        Code::InvalidState, Severity::Error,
        "D3D12 device is not initialized for indirect dispatch.");
  }
  if (!m_dispatchCommandSignature) {
    Result signatureResult = create_dispatch_command_signature(*m_device);
    if (!signatureResult) {
      return signatureResult;
    }
  }
...
DX12GpuCommandContext::set_render_targets(const ViewHandle *renderTargetViews,
  ...
  for (uint32_t i = 0; i < renderTargetCount; ++i) {
    ...
    if (rtvRecord->desc.type != ViewType::RenderTarget) {
      return Result::fail(Code::InvalidArgument, Severity::Error,
                          "The view type of the given render target handle is "
                          "not RenderTarget.");
    }
    uint32_t descriptorIndex = 0;
    Result result =
        resolve_slice_index(rtvRecord->defaultTableIds.size(), descriptorIndex);
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve render target descriptor index "
                          "for the current frame.");
    }
    rtvHandles[i] = m_descriptorAllocator.get_cpu_handle(
        rtvRecord->defaultTableIds[descriptorIndex]);
  }
  ...
  if (depthStencilView.valid()) {
    ...
    if (dsvRecord->desc.type != ViewType::DepthStencil) {
      return Result::fail(Code::InvalidArgument, Severity::Error,
                          "The view type of the given depth stencil handle is "
                          "not DepthStencil.");
    }
    uint32_t descriptorIndex = 0;
    Result result =
        resolve_slice_index(dsvRecord->defaultTableIds.size(), descriptorIndex);
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve depth stencil descriptor index "
                          "for the current frame.");
    }
    dsvHandle = m_descriptorAllocator.get_cpu_handle(
        dsvRecord->defaultTableIds[descriptorIndex]);
  }

  // レンダーターゲットとデプスステンシルをセットする
  m_commandList->OMSetRenderTargets(renderTargetCount, rtvHandles.data(), false,
                                    depthStencilView.valid() ? &dsvHandle
                                                             : nullptr);

  return Result::ok();
}
Result DX12GpuCommandContext::draw_instanced(uint32_t vertexCountPerInstance,
                                             uint32_t instanceCount,
                                             uint32_t startVertexLocation,
                                             uint32_t startInstanceLocation) {
  // draw 呼び出しは graphics queue でのみ有効とし、compute/copy queue
  // での誤発行を防ぐ
  if (type() != CommandListType::Graphics) {
    return Result::fail(
        Code::InvalidArgument, Severity::Error,
  ...
  // 頂点/インスタンス範囲は呼び出し側の宣言どおりにそのまま発行し、pass 実装が
  // draw パターンを選べるようにする
...
Result DX12GpuCommandContext::draw_indexed_instanced(
  ...
  // index 範囲と base vertex は呼び出し側の宣言どおりにそのまま流し、mesh slice
  // 単位の描画を許可する
...
Result DX12GpuCommandContext::execute_instanced_indirect(
    BufferHandle commandBufferHandle, uint32_t maxCommandCount) {
  if (type() != CommandListType::Graphics) {
    return Result::fail(
        Code::InvalidArgument, Severity::Error,
  ...
  if (m_device == nullptr) {
    return Result::fail(Code::InvalidState, Severity::Error,
                        "D3D12 device is not initialized for indirect draw.");
  }
  if (!m_drawCommandSignature) {
    Result signatureResult = create_draw_command_signature(*m_device);
    if (!signatureResult) {
      return signatureResult;
    }
  }
  ...
  m_commandList->ExecuteIndirect(m_drawCommandSignature.Get(), maxCommandCount,
                                 commandResource->get_resource(), 0, nullptr,
                                 0);
  return Result::ok();
}
Result DX12GpuCommandContext::execute_indexed_indirect(
    BufferHandle commandBufferHandle, BufferHandle commandCountBufferHandle,
  ...
  m_commandList->ExecuteIndirect(m_drawIndexedCommandSignature.Get(),
                                 maxCommandCount,
                                 commandResource->get_resource(), 0,
                                 commandCountResource->get_resource(), 0);
  return Result::ok();
}
Result
DX12GpuCommandContext::create_command_allocator(ID3D12Device &device,
                                                D3D12_COMMAND_LIST_TYPE type) {
  // コマンドアロケータの作成
  HRESULT hr =
...
Result
DX12GpuCommandContext::create_dispatch_command_signature(ID3D12Device &device) {
  ...
  HRESULT hr = device.CreateCommandSignature(
      ...
      IID_PPV_ARGS(m_dispatchCommandSignature.ReleaseAndGetAddressOf()));
  if (FAILED(hr)) {
    return Result::fail(PAL::Win::convert_hresult_code(hr), Severity::Error,
                        "Failed to create dispatch command signature.");
  }
...
Result DX12GpuCommandContext::create_timestamp_resources(
  ...
  std::memset(m_timestampReadbackMappedData, 0,
              static_cast<size_t>(resourceDesc.Width));
  return Result::ok();
}
Result DX12GpuCommandContext::resolve_slice_index(size_t sliceCount,
  ...
  // 単一実体のリソースは全フレームで index 0 を共有し、frame-local 実体だけ
  // m_frameIndex で切り替える
  if (sliceCount == 1) {
    outIndex = 0;
    return Result::ok();
  ...
  // Present パスでは m_frameIndex に「現在の back buffer
  // index」が渡されるため、 その index が有効範囲内ならそのまま使いる
  ...
  if (sliceCount != m_bufferCount) {
    return Result::fail(Code::InvalidArgument, Severity::Error,
                        "Slice count must be 1, match the global buffer count, "
                        "or contain the current frame index.");
  }
  return Result::fail(
      Code::InvalidArgument, Severity::Error,
      "Frame index is out of range for the requested resource slice.");
}
Result DX12GpuCommandContext::resolve_root_descriptor_buffer(
  ...
  *outResource = nullptr;
  ...
  if (!record->defaultResources.empty()) {
    ...
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve default buffer slice index for "
  ...
  if (!record->uploadResources.empty()) {
    uint32_t resourceIndex = 0;
    Result result =
        resolve_slice_index(record->uploadResources.size(), resourceIndex);
    if (!result) {
      return Result::fail(result.code, Severity::Error,
                          "Failed to resolve upload buffer slice index for "
                          "root descriptor binding.");
    }

    *outResource =
        const_cast<DX12GpuResource *>(&record->uploadResources[resourceIndex]);
    return Result::ok();
  }
...
Result DX12GpuCommandContext::resolve_upload_buffer(
...
Result DX12GpuCommandContext::resolve_default_buffer(
    BufferHandle handle, uint32_t resourceIndex,
    DX12GpuResource **outResource) const {
  *outResource = nullptr;
...
Result
DX12GpuCommandContext::create_command_list(ID3D12Device &device,
                                           D3D12_COMMAND_LIST_TYPE type) {
  // コマンドリストの作成
  HRESULT hr = device.CreateCommandList(0, type, m_commandAllocator.Get(),
...
DX12GpuCommandQueue::DX12GpuCommandQueue(ID3D12Device &device,
                                         D3D12_COMMAND_LIST_TYPE type) {
  create_fence(device);
  create_fence_event();
  create_command_queue(device, type);
}
CommandListType DX12GpuCommandQueue::type() const { return m_type; }
Result DX12GpuCommandQueue::submit(std::vector<ICommandContext *> &contexts) {
  // RHI の command context 群から native command list を取り出し、同じ queue
  // へまとめて投入する。
  ...
  for (ICommandContext *context : contexts) {
    ...
    DX12GpuCommandContext &dx12Cmd =
        static_cast<DX12GpuCommandContext &>(*context);
  ...
  m_commandQueue->ExecuteCommandLists(static_cast<UINT>(commandLists.size()),
                                      commandLists.data());
  return Result::ok();
}
...
Result DX12GpuCommandQueue::wait_for_queue(IQueueContext &queue) {
  ...
  const HRESULT hr =
      m_commandQueue->Wait(dx12Queue.m_fence.Get(), dx12Queue.m_fenceValue);
  if (FAILED(hr)) {
    return Result::fail(PAL::Win::convert_hresult_code(hr), Severity::Error,
...
Result
DX12GpuCommandQueue::get_timestamp_frequency(uint64_t &outFrequency) const {
  outFrequency = 0;
  if (!m_commandQueue) {
    return Result::fail(Code::InvalidState, Severity::Error,
...
Result DX12GpuCommandQueue::create_fence(ID3D12Device &device) {
  ...
  m_fence.Reset();
  m_fenceValue = 0; // 初期値0
  HRESULT hr = device.CreateFence(m_fenceValue, D3D12_FENCE_FLAG_NONE,
...
Result DX12GpuCommandQueue::create_command_queue(ID3D12Device &device,
                                                 D3D12_COMMAND_LIST_TYPE type) {
...
Result DX12CommandPool::get_command_context(CommandListType type,
                                            commandContextLease &outContext) {
  ...
  case Cue::RHI::CommandListType::Graphics: {
    ...
    outContext = commandContextLease(context.release(),
...
Result DX12CommandPool::return_command_context(commandContextLease &context) {
  // fence 未完了の context は pending 配列へ退避し、完了済みなら即 pool
  // に戻す。 lease はここで空にして、呼び出し側の二重返却を防ぐ。
  if (!context) {
    return Result::ok();
  }
...
bool DX12CommandPool::try_recycle_completed_contexts(
  ...
  for (size_t readIndex = 0; readIndex < pendingContexts.size(); ++readIndex) {
    ...
    pool.recycle(
        static_cast<DX12GpuCommandContext *>(pendingContext.release()));
    didRecycle = true;
  }
...
void DX12CommandPool::recycle_completed_graphics_contexts_locked() noexcept {
  (void)try_recycle_completed_contexts(m_pendingGraphicsContexts,
                                       m_graphicsContextPool);
}
void DX12CommandPool::recycle_completed_compute_contexts_locked() noexcept {
  (void)try_recycle_completed_contexts(m_pendingComputeContexts,
                                       m_computeContextPool);
}
void DX12CommandPool::recycle_completed_copy_contexts_locked() noexcept {
  (void)try_recycle_completed_contexts(m_pendingCopyContexts,
                                       m_copyContextPool);
}
Result DX12QueuePool::get_queue_context(CommandListType type,
                                        queueContextLease &outContext) {
  switch (type) {
  case Cue::RHI::CommandListType::Graphics: {
    std::lock_guard lock(m_graphicsPoolMutex);
    // グラフィックスコマンドキューコンテキストをプールから取得
    auto context = m_graphicsQueuePool.acquire();
    outContext = queueContextLease(context.release(),
...
Result DX12QueuePool::return_queue_context(queueContextLease &context) {
  ...
  case Cue::RHI::CommandListType::Graphics: {
    ...
    std::lock_guard lock(m_graphicsPoolMutex);
    m_graphicsQueuePool.recycle(
        static_cast<DX12GpuCommandQueue *>(context.release()));
  } break;
  case Cue::RHI::CommandListType::Compute: {
    // コンピュートコマンドキューコンテキストをプールへ返却
    std::lock_guard lock(m_computePoolMutex);
    m_computeQueuePool.recycle(
        static_cast<DX12GpuCommandQueue *>(context.release()));
  } break;
  case Cue::RHI::CommandListType::Copy: {
    // コピーコマンドキューコンテキストをプールへ返却
    std::lock_guard lock(m_copyPoolMutex);
    m_copyQueuePool.recycle(
        static_cast<DX12GpuCommandQueue *>(context.release()));
  } break;
  default:
    break;
