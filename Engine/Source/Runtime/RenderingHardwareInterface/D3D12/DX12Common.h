...
inline void set_dxgi_name([[maybe_unused]] IDXGIObject *a_obj,
                          [[maybe_unused]] const wchar_t *a_name) {
  if (a_obj) {
    a_obj->SetPrivateData(
        WKPDID_D3DDebugObjectName,
        static_cast<UINT>((wcslen(a_name) + 1) * sizeof(wchar_t)), a_name);
  }
}

inline void set_d3d12_name([[maybe_unused]] ID3D12Object *a_obj,
                           [[maybe_unused]] const wchar_t *a_name) {
  if (a_obj) {
...
inline ColorFormat convert_color_format(DXGI_FORMAT format) {
  ...
  case DXGI_FORMAT_R32G32_UINT:
    return ColorFormat::R32G32_UINT;
...
convert_primitive_topology_type(PrimitiveTopologyType type) {
  switch (type) {
...
convert_primitive_topology(PrimitiveTopologyType type) {
  switch (type) {
  case PrimitiveTopologyType::Point:
