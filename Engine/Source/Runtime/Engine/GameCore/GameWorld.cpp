...
[[nodiscard]] DrawSystem::CpuShadowCaster make_cpu_shadow_caster(
  ...
  const Math::float3 basisX(world.values[0][0], world.values[0][1],
                            world.values[0][2]);
  const Math::float3 basisY(world.values[1][0], world.values[1][1],
                            world.values[1][2]);
  const Math::float3 basisZ(world.values[2][0], world.values[2][1],
                            world.values[2][2]);
  ...
  caster.center =
      Math::float3(world.values[3][0], world.values[3][1], world.values[3][2]);
  caster.radius = maxExtent * 1.7320508f;
  return caster;
}

[[nodiscard]] Math::float3
transform_position(const GpuData::ObjectTransformGpu &a_transform) noexcept {
  const Math::float4x4 &world = a_transform.worldMatrix;
  return Math::float3(world.values[3][0], world.values[3][1],
                      world.values[3][2]);
}

[[nodiscard]] Math::float3
camera_position(const GpuData::ViewProjectionGpu &a_viewProjection) noexcept {
...
make_default_static_mesh_object_definition(const Math::float3 &a_position,
                                           uint32_t a_meshId) {
...
[[nodiscard]] ObjectDefinition
make_default_game_object_definition(const Math::float3 &a_position) {
...
[[nodiscard]] ObjectDefinition
make_default_camera_object_definition(const Math::float3 &a_position) {
...
[[nodiscard]] ObjectDefinition
make_default_sprite_object_definition(const Math::float3 &a_position,
...
[[nodiscard]] ObjectDefinition
make_default_directional_light_definition(const Math::float3 &a_position) {
  ObjectDefinition objectDefinition("DirectionalLight");

  ECS::TransformComponent transform{};
  transform.position = a_position;
  transform.rotation =
      Math::quaternion_from_euler_xyz(Math::float3(0.6f, -0.5f, 0.0f));
  transform.scale = Math::float3(1.0f, 1.0f, 1.0f);
  objectDefinition.prototype.add_component(transform);
...
[[nodiscard]] ObjectDefinition
make_default_point_light_definition(const Math::float3 &a_position) {
...
[[nodiscard]] ObjectDefinition
make_default_spot_light_definition(const Math::float3 &a_position) {
  ObjectDefinition objectDefinition("SpotLight");

  ECS::TransformComponent transform{};
  transform.position = a_position;
  transform.rotation =
      Math::quaternion_from_euler_xyz(Math::float3(0.35f, 0.0f, 0.0f));
  transform.scale = Math::float3(1.0f, 1.0f, 1.0f);
  objectDefinition.prototype.add_component(transform);
...
[[nodiscard]] Result GameWorld::initialize(
    ...
    Physics::IPhysicsSystem *a_physicsSystem, PAL::InputManager *a_inputManager,
    uint32_t a_bufferCount, uint32_t a_renderWidth, uint32_t a_renderHeight,
  ...
  Result result =
      m_drawResources->create_renderable_info_buffer(k_maxRenderObjectCount);
  if (!result) {
    return result;
  }
  ...
  result =
      m_particleResources->create_emitter_buffer(k_maxParticleEmitterCount);
  if (!result) {
    return result;
  }
  ...
  result = m_particleResources->create_trail_buffer(
      k_maxParticleCount, GpuData::k_maxParticleTrailSegmentCount);
  if (!result) {
    return result;
  }

  m_effectPrimitiveResources =
      std::make_unique<EffectSystem::EffectPrimitiveResources>(a_bufferManager,
                                                               a_bufferCount);
  ...
  result = m_effectPrimitiveResources->create_sprite_buffer(
      GpuData::k_maxEffectSpriteCount);
  if (!result) {
    return result;
  }

  result = m_effectPrimitiveResources->create_ribbon_buffer(
  ...
  result =
      m_lightResources->create_spot_light_buffer(GpuData::k_maxSpotLightCount);
  if (!result) {
    return result;
  }
  ...
  auto &renderableObjectSystem = m_ecs.add_system<ECS::RenderableObjectSystem>(
      m_assetManager, a_staticMeshPool, m_defaultMaterialHandle,
  ...
  auto &textSystem = m_ecs.add_system<ECS::TextSystem>(
      m_fontAtlasManager, m_drawFrameState, m_drawScene);
  auto &particleEmitterSystem = m_ecs.add_system<ECS::ParticleEmitterSystem>(
      m_assetManager, m_defaultMaterialHandle, m_particleRangeAllocator,
      m_particleScene);
  auto &effectEmitterSystem = m_ecs.add_system<ECS::EffectEmitterSystem>(
      m_assetManager, m_defaultMaterialHandle, m_particleRangeAllocator,
      m_particleScene, m_effectPrimitiveScene);
  auto &uiLayoutSystem =
      m_ecs.add_system<ECS::UiLayoutSystem>(m_drawFrameState);
  auto &uiWidgetSystem = m_ecs.add_system<ECS::UiWidgetSystem>(
      m_inputManager, m_assetManager, m_defaultMaterialHandle, m_drawFrameState,
      m_drawScene);
  auto &cameraSystem =
      m_ecs.add_system<ECS::CameraSystem>(m_drawFrameState, m_drawScene);
  ...
  auto &navigationSystem =
      m_ecs.add_system<ECS::NavigationSystem>(&m_navigationWorld);
...
[[nodiscard]] Result GameWorld::editor_update(uint32_t a_bufferIndex,
...
[[nodiscard]] Result GameWorld::update(float a_deltaTime,
...
[[nodiscard]] Result GameWorld::clone_from(const GameWorld &a_source) {
  ...
  for (const SceneId sceneId : sceneIds) {
    ...
    try {
      (void)instantiate_into_scene(
          sceneId, std::span<const ObjectDefinition>(objectDefinitions),
          sourceScene.asset);
  ...
  for (const PendingLooseParent &pendingParent : pendingLooseParents) {
    ...
    BaseComponent *base =
        get_component<BaseComponent>(pendingParent.targetEntityId);
    if (base == nullptr) {
      return Result::fail(Code::InvalidState, Severity::Error,
...
[[nodiscard]] Result GameWorld::add_object(const Math::float3 &a_position,
  ...
  result = add_component<ECS::TransformComponent>(a_outObject.entity_id(),
                                                  transform);
  if (!result || transform == nullptr) {
    destroy_object_immediately(a_outObject.entity_id());
    return result
               ? Result::fail(Code::CreateFailed, Severity::Error,
                              "Failed to add transform component for object.")
               : result;
  }

  ECS::MeshFilterComponent *meshFilter = nullptr;
  result = add_component<ECS::MeshFilterComponent>(a_outObject.entity_id(),
                                                   meshFilter);
  ...
  if (!result || renderer == nullptr) {
    ...
    return result
               ? Result::fail(
                     Code::CreateFailed, Severity::Error,
...
[[nodiscard]] Result
GameWorld::add_camera_object(const Math::float3 &a_position,
                             GameObject &a_outObject) {
  a_outObject = {};
...
[[nodiscard]] Result
GameWorld::add_sprite_object(const Math::float3 &a_position,
                             GameObject &a_outObject) {
  a_outObject = {};
  ...
  result = add_component<ECS::SpriteRendererComponent>(a_outObject.entity_id(),
                                                       renderer);
  if (!result || renderer == nullptr) {
    destroy_object_immediately(a_outObject.entity_id());
    return result ? Result::fail(
...
[[nodiscard]] Result
GameWorld::add_directional_light_object(const Math::float3 &a_position,
                                        GameObject &a_outObject) {
  a_outObject = {};
...
[[nodiscard]] Result
GameWorld::add_point_light_object(const Math::float3 &a_position,
                                  GameObject &a_outObject) {
  a_outObject = {};
  ...
  result =
      add_component<ECS::PointLightComponent>(a_outObject.entity_id(), light);
  if (!result || light == nullptr) {
    destroy_object_immediately(a_outObject.entity_id());
    return result ? Result::fail(Code::CreateFailed, Severity::Error,
...
[[nodiscard]] Result
GameWorld::add_spot_light_object(const Math::float3 &a_position,
                                 GameObject &a_outObject) {
  a_outObject = {};
  ...
  result =
      add_component<ECS::SpotLightComponent>(a_outObject.entity_id(), light);
  if (!result || light == nullptr) {
    destroy_object_immediately(a_outObject.entity_id());
    return result ? Result::fail(Code::CreateFailed, Severity::Error,
...
[[nodiscard]] Result
GameWorld::add_object_to_scene(SceneId a_sceneId,
  ...
  const ObjectDefinition objectDefinition =
      make_default_static_mesh_object_definition(a_position,
                                                 m_defaultStaticMeshId);
  return append_object_to_scene(a_sceneId, objectDefinition, a_outObject);
}

[[nodiscard]] Result
GameWorld::add_game_object_to_scene(SceneId a_sceneId,
                                    const Math::float3 &a_position,
                                    GameObject &a_outObject) {
  a_outObject = {};
  if (a_sceneId == k_invalidSceneId) {
...
[[nodiscard]] Result
GameWorld::add_camera_object_to_scene(SceneId a_sceneId,
                                      const Math::float3 &a_position,
                                      GameObject &a_outObject) {
  a_outObject = {};
  if (a_sceneId == k_invalidSceneId) {
...
[[nodiscard]] Result
GameWorld::add_sprite_object_to_scene(SceneId a_sceneId,
                                      const Math::float3 &a_position,
                                      GameObject &a_outObject) {
  a_outObject = {};
  if (a_sceneId == k_invalidSceneId) {
  ...
  const ObjectDefinition objectDefinition =
      make_default_sprite_object_definition(a_position,
                                            m_defaultMaterialHandle);
  return append_object_to_scene(a_sceneId, objectDefinition, a_outObject);
}

[[nodiscard]] Result
GameWorld::add_directional_light_object_to_scene(SceneId a_sceneId,
                                                 const Math::float3 &a_position,
                                                 GameObject &a_outObject) {
  a_outObject = {};
  if (a_sceneId == k_invalidSceneId) {
...
[[nodiscard]] Result
GameWorld::add_point_light_object_to_scene(SceneId a_sceneId,
                                           const Math::float3 &a_position,
                                           GameObject &a_outObject) {
  a_outObject = {};
  if (a_sceneId == k_invalidSceneId) {
...
[[nodiscard]] Result
GameWorld::add_spot_light_object_to_scene(SceneId a_sceneId,
                                          const Math::float3 &a_position,
                                          GameObject &a_outObject) {
  a_outObject = {};
  if (a_sceneId == k_invalidSceneId) {
...
[[nodiscard]] Result GameWorld::upload_draw_scene(uint32_t a_bufferIndex) {
  ...
  const Math::float3 currentCameraPosition =
      sortCamera != nullptr ? camera_position(sortCamera->viewProjection)
                            : Math::float3::zero();
  ...
  if (frameState.useCpuBatching) {
    ...
    std::stable_sort(frameState.transparentCpuIndexedDraws.begin(),
  ...
  if (!renderableInfoUploaders.empty()) {
    ...
    if (uploaderIndex < renderableInfoUploaders.size()) {
      ...
      for (uint32_t index = 0;
           index < sceneFrame.staticMeshVisibilityItems.size(); ++index) {
        if (!uploader.push(
                index,
  ...
  if (!transformUploaders.empty()) {
    ...
    if (uploaderIndex < transformUploaders.size()) {
      ...
      uploader.begin_frame();
      for (uint32_t index = 0; index < sceneFrame.staticMeshSurfaceItems.size();
           ++index) {
        if (!uploader.push(
                index, sceneFrame.staticMeshSurfaceItems[index].transform)) {
          return Result::fail(Code::InvalidState, Severity::Error,
  ...
  std::stable_sort(sceneFrame.spriteItems.begin(), sceneFrame.spriteItems.end(),
...
[[nodiscard]] Result GameWorld::upload_particle_scene(uint32_t a_bufferIndex) {
  ...
  frameState.frame.emitterCount =
      (std::min)(static_cast<uint32_t>(sceneFrame.emitters.size()),
  ...
  for (uint32_t emitterIndex = 0; emitterIndex < frameState.frame.emitterCount;
    ...
    particleCount = (std::max)(particleCount,
  ...
  m_particleTrailFrameIndex = (m_particleTrailFrameIndex + 1u) %
                              GpuData::k_maxParticleTrailSegmentCount;
...
[[nodiscard]] Result
GameWorld::upload_effect_primitive_scene(uint32_t a_bufferIndex) {
  ...
  EffectSystem::EffectPrimitiveFrameData &frameState =
      m_effectPrimitiveFrameState.frame_state(a_bufferIndex);
  frameState.frame.spriteCount =
      (std::min)(static_cast<uint32_t>(sceneFrame.sprites.size()),
                 GpuData::k_maxEffectSpriteCount);
  frameState.frame.ribbonCount =
      (std::min)(static_cast<uint32_t>(sceneFrame.ribbons.size()),
                 GpuData::k_maxEffectRibbonCount);
...
[[nodiscard]] Result GameWorld::upload_light_scene(uint32_t a_bufferIndex) {
  ...
  if (!directionalUploaders.empty()) {
    ...
    if (uploaderIndex < directionalUploaders.size()) {
      ...
      uploader.begin_frame();
      for (uint32_t index = 0; index < frameState.frame.directionalLightCount;
           ++index) {
  ...
  if (!pointUploaders.empty()) {
    ...
    if (uploaderIndex < pointUploaders.size()) {
      ...
      uploader.begin_frame();
      for (uint32_t index = 0; index < frameState.frame.pointLightCount;
           ++index) {
  ...
  if (!spotUploaders.empty()) {
    ...
    if (uploaderIndex < spotUploaders.size()) {
      ...
      uploader.begin_frame();
      for (uint32_t index = 0; index < frameState.frame.spotLightCount;
           ++index) {
...
[[nodiscard]] Result GameWorld::upload_shadow_scene(uint32_t a_bufferIndex) {
  ...
  frameState.pointShadowFaces =
      sceneFrame.hasPointShadow ? sceneFrame.pointShadow.faces
  ...
  const uint32_t uploaderIndex =
      resolve_uploader_index(static_cast<uint32_t>(uploaders.size()));
  ...
  for (uint32_t faceIndex = 0; faceIndex < GpuData::k_pointShadowFaceCount;
       ++faceIndex) {
    if (!pointUploader.push(faceIndex,
                            frameState.pointShadowFaces[faceIndex])) {
      return Result::fail(Code::InvalidState, Severity::Error,
