...
{
    GameObject::GameObject(GameWorld* a_world, EntityId a_entityId,
    ...
    template <typename T>
    Result GameObject::get_component(T*& a_outComponent) noexcept
    {
        if (!is_valid())
        {
    ...
    Result GameObject::add_component(T*& a_outComponent, Args &&...a_args)
    {
        if (!is_valid())
        {
            a_outComponent = nullptr;
