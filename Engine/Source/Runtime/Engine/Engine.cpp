...
#include "EffectSystem/Passes/EffectPrimitiveBufferCopyPass.h"
#include "EffectSystem/Passes/EffectRibbonForwardPass.h"
#include "EffectSystem/Passes/EffectSpriteForwardPass.h"
...
void Engine::shutdown() {
  ...
  if (!result) {
    CUE_ASSERTF(false, "Failed to destroy debug view projection buffer: %s",
                result.message.data());
  }
  ...
  if (!result) {
    CUE_ASSERTF(false, "Failed to destroy debug selection buffer: %s",
                result.message.data());
  }
...
Result Engine::create_render_target_resources(
    std::string_view a_name, RHI::ColorFormat a_format,
    RenderTargetResources &a_outResources, const float *a_clearColor) {
  auto *textureManager = m_backend->get_texture_manager();
  auto *viewManager = m_backend->get_view_manager();
  ...
  result =
      viewManager->create_view(colorRtvDesc, a_outResources.colorRtvHandle);
  if (!result) {
    return result;
  }
  ...
  result =
      viewManager->create_view(colorSrvDesc, a_outResources.colorSrvHandle);
  if (!result) {
    return result;
  }
...
Result
Engine::destroy_render_target_resources(RenderTargetResources &a_resources) {
  ...
  if (viewManager != nullptr) {
    if (a_resources.colorSrvHandle.valid()) {
      Result result = viewManager->destroy_view(a_resources.colorSrvHandle);
      if (!result) {
        return result;
      }
    ...
    if (a_resources.colorRtvHandle.valid()) {
      Result result = viewManager->destroy_view(a_resources.colorRtvHandle);
      if (!result) {
        return result;
      }
  ...
  if (textureManager != nullptr && a_resources.colorHandle.valid()) {
    Result result = textureManager->destroy_texture(a_resources.colorHandle);
    if (!result) {
      return result;
    }
...
Result Engine::create_debug_pick_readback_buffer() {
  ...
  Result result =
      bufferManager->create_buffer(bufferDesc, m_debugPickReadbackBufferHandle);
  if (!result) {
    return result;
  }

  result = bufferManager->get_readback_buffer_view(
      m_debugPickReadbackBufferHandle, m_debugPickReadbackView);
  if (!result) {
    return result;
  }
...
Result Engine::create_view_projection_buffer(
    std::string_view a_name, RHI::BufferHandle &a_outBufferHandle,
...
Result Engine::create_debug_selection_buffer() {
  ...
  Result result =
      bufferManager->create_buffer(bufferDesc, m_debugSelectionBufferHandle);
  if (!result) {
    return result;
  }

  result = bufferManager->create_slot_uploaders(m_debugSelectionBufferHandle,
...
Result Engine::destroy_debug_selection_buffer() {
  ...
  if (bufferManager != nullptr && m_debugSelectionBufferHandle.valid()) {
    Result result = bufferManager->destroy_buffer(m_debugSelectionBufferHandle);
    if (!result) {
      return result;
    }
...
bool Engine::request_debug_pick(float a_normalizedX,
  ...
  const float x = (std::max)(0.0f, (std::min)(a_normalizedX, 1.0f));
  const float y = (std::max)(0.0f, (std::min)(a_normalizedY, 1.0f));
  const uint32_t width = (std::max)(m_backend->width(), 1u);
  const uint32_t height = (std::max)(m_backend->height(), 1u);
  m_debugPickState.x = (std::min)(static_cast<uint32_t>(x * width), width - 1u);
  m_debugPickState.y =
...
void Engine::resolve_debug_pick_readback() noexcept {
  ...
  const std::byte *mappedData =
      m_debugPickReadbackView
          .mappedDatas[m_debugPickState.readbackResourceIndex];
  ...
  if (encodedObjectId > 0 && m_activeWorld != nullptr) {
    const Result result =
        m_activeWorld->get_render_object_entity(encodedObjectId - 1u, entityId);
    if (!result) {
...
Result Engine::recreate_render_frame_graph() {
  ...
  const DrawSystem::DrawResources *drawResources =
      m_activeWorld->draw_resources();
  ...
  const EffectSystem::EffectPrimitiveResources *effectPrimitiveResources =
      m_activeWorld->effect_primitive_resources();
  if (effectPrimitiveResources == nullptr) {
    return Result::fail(
        Code::InvalidState, Severity::Error,
        "Engine effect primitive resources are not initialized.");
  }
  auto *bufferManager = m_backend->get_buffer_manager();
  if (bufferManager == nullptr) {
  ...
  m_frameGraph->add_pass(std::make_unique<DrawSystem::ViewProjectionCopyPass>(
      m_debugViewProjectionBufferHandle, sizeof(GpuData::ViewProjectionGpu),
      "DebugViewProjectionCopy"));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::ViewProjectionCopyPass>(
      m_effectPreviewViewProjectionBufferHandle,
      sizeof(GpuData::ViewProjectionGpu), "EffectPreviewViewProjectionCopy"));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::ViewProjectionCopyPass>(
      m_debugSelectionBufferHandle, sizeof(GpuData::DebugSelectionGpu),
      "DebugSelectionCopy"));
  ...
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleBufferCopyPass>(
          m_activeWorld->particle_frame_state(),
          particleResources->frame_buffer_handle(),
          particleResources->emitter_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectPrimitiveBufferCopyPass>(
          m_activeWorld->effect_primitive_frame_state(),
          effectPrimitiveResources->frame_buffer_handle(),
          effectPrimitiveResources->sprite_buffer_handle(),
          effectPrimitiveResources->ribbon_buffer_handle()));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::RenderObjectCopyPass>(
      m_activeWorld->draw_frame_state(),
      drawResources->render_object_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<DrawSystem::VisibleObjectCountCopyPass>(
          m_activeWorld->draw_frame_state(),
          drawResources->visible_object_count_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleInitializePass>(
  ...
  m_frameGraph->add_pass(std::make_unique<ParticleSystem::ParticleUpdatePass>(
      ...
      particleResources->particle_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleTrailUpdatePass>(
          m_activeWorld->particle_frame_state(),
          particleResources->frame_buffer_handle(),
          particleResources->particle_buffer_handle(),
          particleResources->trail_buffer_handle()));
  m_frameGraph->add_pass(std::make_unique<LightingSystem::LightBufferCopyPass>(
      "LightFrameBufferCopy", lightingBindings.frameBuffer,
      sizeof(GpuData::LightFrameGpu)));
  m_frameGraph->add_pass(std::make_unique<LightingSystem::LightBufferCopyPass>(
      "DirectionalLightBufferCopy", lightingBindings.directionalLightBuffer,
      static_cast<uint64_t>(GpuData::k_maxDirectionalLightCount) *
          sizeof(GpuData::DirectionalLightGpu)));
  m_frameGraph->add_pass(std::make_unique<LightingSystem::LightBufferCopyPass>(
      "PointLightBufferCopy", lightingBindings.pointLightBuffer,
      static_cast<uint64_t>(GpuData::k_maxPointLightCount) *
          sizeof(GpuData::PointLightGpu)));
  m_frameGraph->add_pass(std::make_unique<LightingSystem::LightBufferCopyPass>(
      "SpotLightBufferCopy", lightingBindings.spotLightBuffer,
  ...
  m_frameGraph->add_pass(std::make_unique<ShadowSystem::ShadowBufferCopyPass>(
      "PointShadowFaceBufferCopy", shadowBindings.pointShadowFaceBuffer,
      static_cast<uint64_t>(GpuData::k_pointShadowFaceCount) *
          sizeof(GpuData::PointShadowFaceGpu)));
  m_frameGraph->add_pass(std::make_unique<ShadowSystem::ShadowBufferCopyPass>(
      "SpotShadowFrameBufferCopy", shadowBindings.spotShadowFrameBuffer,
  ...
  m_frameGraph->add_pass(
      std::make_unique<DrawSystem::VisibleObjectBucketCountPass>(
          m_activeWorld->draw_frame_state(),
          drawResources->render_object_buffer_handle(),
          drawResources->visible_object_count_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<DrawSystem::VisibleObjectBucketPrefixPass>(
          m_activeWorld->draw_frame_state()));
  m_frameGraph->add_pass(
      std::make_unique<DrawSystem::VisibleObjectBucketScatterPass>(
          m_activeWorld->draw_frame_state(),
          drawResources->render_object_buffer_handle(),
          drawResources->visible_object_count_buffer_handle()));
  ...
  m_frameGraph->add_pass(
      std::make_unique<ShadowSystem::DirectionalShadowMapPass>(
          ...
          drawResources->transform_buffer_handle(),
          drawResources->visible_object_count_buffer_handle(),
          drawResources->skin_palette_buffer_handle(), shadowBindings,
          m_cubeIndexCount));
  ...
  m_frameGraph->add_pass(std::make_unique<ShadowSystem::SpotShadowMapPass>(
      m_activeWorld->draw_frame_state(), m_activeWorld->shadow_frame_state(),
      drawResources->render_object_buffer_handle(),
      drawResources->transform_buffer_handle(),
      drawResources->visible_object_count_buffer_handle(),
      drawResources->skin_palette_buffer_handle(), shadowBindings,
      m_cubeIndexCount));
  m_frameGraph->add_pass(
      std::make_unique<ShadowSystem::SpotShadowMapPreviewPass>(shadowBindings));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::StaticMeshForwardPass>(
      "GameOpaqueStaticMeshForward", "GameColor", "GameColorRTV",
      "GameSceneDepth", "GameSceneDepthDSV", m_activeWorld->draw_frame_state(),
      drawResources->render_object_buffer_handle(),
      drawResources->transform_buffer_handle(),
      drawResources->view_projection_buffer_handle(),
      drawResources->visible_object_count_buffer_handle(),
      drawResources->material_buffer_handle(),
      drawResources->skin_palette_buffer_handle(), lightingBindings,
      shadowBindings, m_cubeIndexCount, m_skyboxTextureSrvHandle));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::SkyboxPass>(
      "GameSkybox", "GameColor", "GameColorRTV", "GameSceneDepth",
      "GameSceneDepthDSV", drawResources->view_projection_buffer_handle(),
      m_skyboxTextureSrvHandle));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::StaticMeshForwardPass>(
      "GameTransparentStaticMeshForward", "GameColor", "GameColorRTV",
      "GameSceneDepth", "GameSceneDepthDSV", m_activeWorld->draw_frame_state(),
      drawResources->render_object_buffer_handle(),
      drawResources->transform_buffer_handle(),
      drawResources->view_projection_buffer_handle(),
      ...
      drawResources->material_buffer_handle(),
      drawResources->skin_palette_buffer_handle(), lightingBindings,
      shadowBindings, m_cubeIndexCount, m_skyboxTextureSrvHandle, nullptr,
      DrawSystem::StaticMeshForwardQueue::Transparent));
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleSpriteForwardPass>(
          "GameParticleSpriteForward", "GameColor", "GameColorRTV",
          "GameSceneDepth", "GameSceneDepthDSV",
          m_activeWorld->particle_frame_state(),
          drawResources->view_projection_buffer_handle(),
          particleResources->particle_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleTrailForwardPass>(
          "GameParticleTrailForward", "GameColor", "GameColorRTV",
          "GameSceneDepth", "GameSceneDepthDSV",
          m_activeWorld->particle_frame_state(),
          particleResources->frame_buffer_handle(),
          drawResources->view_projection_buffer_handle(),
  ...
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleMeshForwardPass>(
          "GameParticleMeshForward", "GameColor", "GameColorRTV",
          "GameSceneDepth", "GameSceneDepthDSV",
          m_activeWorld->particle_frame_state(),
          drawResources->view_projection_buffer_handle(),
          particleResources->particle_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectSpriteForwardPass>(
          "GameEffectSpriteAlphaForward", "GameColor", "GameColorRTV",
          "GameSceneDepth", "GameSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          drawResources->view_projection_buffer_handle(),
          effectPrimitiveResources->sprite_buffer_handle(),
          EffectSystem::EffectBlendMode::Alpha));
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectSpriteForwardPass>(
          "GameEffectSpriteAdditiveForward", "GameColor", "GameColorRTV",
          "GameSceneDepth", "GameSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          drawResources->view_projection_buffer_handle(),
          effectPrimitiveResources->sprite_buffer_handle(),
          EffectSystem::EffectBlendMode::Additive));
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectRibbonForwardPass>(
          "GameEffectRibbonAlphaForward", "GameColor", "GameColorRTV",
          "GameSceneDepth", "GameSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          drawResources->view_projection_buffer_handle(),
          effectPrimitiveResources->ribbon_buffer_handle(),
  ...
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectRibbonForwardPass>(
          "GameEffectRibbonAdditiveForward", "GameColor", "GameColorRTV",
          "GameSceneDepth", "GameSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          drawResources->view_projection_buffer_handle(),
          effectPrimitiveResources->ribbon_buffer_handle(),
  ...
  m_frameGraph->add_pass(std::make_unique<DrawSystem::SpriteForwardPass>(
      "GameSpriteForward", "GameColor", "GameColorRTV",
      m_activeWorld->draw_frame_state(),
      drawResources->sprite_instance_buffer_handle()));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::DebugDrawPass>(
      "GameDebugDraw", "GameColor", "GameColorRTV", "GameSceneDepth",
      "GameSceneDepthDSV", *m_activeWorld, *bufferManager,
      drawResources->view_projection_buffer_handle()));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::StaticMeshForwardPass>(
      "DebugOpaqueStaticMeshForward", "DebugColor", "DebugColorRTV",
      "DebugSceneDepth", "DebugSceneDepthDSV",
      m_activeWorld->draw_frame_state(),
      drawResources->render_object_buffer_handle(),
      drawResources->transform_buffer_handle(),
      m_debugViewProjectionBufferHandle,
  ...
  m_frameGraph->add_pass(std::make_unique<DrawSystem::SkyboxPass>(
      "DebugSkybox", "DebugColor", "DebugColorRTV", "DebugSceneDepth",
      "DebugSceneDepthDSV", m_debugViewProjectionBufferHandle,
      m_skyboxTextureSrvHandle));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::StaticMeshForwardPass>(
      "DebugTransparentStaticMeshForward", "DebugColor", "DebugColorRTV",
      "DebugSceneDepth", "DebugSceneDepthDSV",
      m_activeWorld->draw_frame_state(),
      drawResources->render_object_buffer_handle(),
      drawResources->transform_buffer_handle(),
  ...
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleSpriteForwardPass>(
          "DebugParticleSpriteForward", "DebugColor", "DebugColorRTV",
          "DebugSceneDepth", "DebugSceneDepthDSV",
          m_activeWorld->particle_frame_state(),
          m_debugViewProjectionBufferHandle,
          particleResources->particle_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleTrailForwardPass>(
          "DebugParticleTrailForward", "DebugColor", "DebugColorRTV",
          "DebugSceneDepth", "DebugSceneDepthDSV",
          m_activeWorld->particle_frame_state(),
          particleResources->frame_buffer_handle(),
          m_debugViewProjectionBufferHandle,
  ...
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleMeshForwardPass>(
          "DebugParticleMeshForward", "DebugColor", "DebugColorRTV",
          "DebugSceneDepth", "DebugSceneDepthDSV",
          m_activeWorld->particle_frame_state(),
          m_debugViewProjectionBufferHandle,
          particleResources->particle_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectSpriteForwardPass>(
          "DebugEffectSpriteAlphaForward", "DebugColor", "DebugColorRTV",
          "DebugSceneDepth", "DebugSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_debugViewProjectionBufferHandle,
          effectPrimitiveResources->sprite_buffer_handle(),
  ...
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectSpriteForwardPass>(
          "DebugEffectSpriteAdditiveForward", "DebugColor", "DebugColorRTV",
          "DebugSceneDepth", "DebugSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_debugViewProjectionBufferHandle,
          effectPrimitiveResources->sprite_buffer_handle(),
  ...
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectRibbonForwardPass>(
          "DebugEffectRibbonAlphaForward", "DebugColor", "DebugColorRTV",
          "DebugSceneDepth", "DebugSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_debugViewProjectionBufferHandle,
          effectPrimitiveResources->ribbon_buffer_handle(),
  ...
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectRibbonForwardPass>(
          "DebugEffectRibbonAdditiveForward", "DebugColor", "DebugColorRTV",
          "DebugSceneDepth", "DebugSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_debugViewProjectionBufferHandle,
          effectPrimitiveResources->ribbon_buffer_handle(),
  ...
  m_frameGraph->add_pass(std::make_unique<DrawSystem::DebugObjectIdPass>(
      ...
      m_debugViewProjectionBufferHandle,
      drawResources->visible_object_count_buffer_handle(),
      drawResources->skin_palette_buffer_handle(), m_cubeIndexCount,
      m_debugSelectedObjectId));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::DebugGridPass>(
      m_debugViewProjectionBufferHandle, m_isDebugGridVisible));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::DebugDrawPass>(
      "DebugViewDebugDraw", "DebugColor", "DebugColorRTV", "DebugSceneDepth",
      "DebugSceneDepthDSV", *m_activeWorld, *bufferManager,
      m_debugViewProjectionBufferHandle));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::DebugSelectionPass>(
      m_debugViewProjectionBufferHandle, m_debugSelectionBufferHandle));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::SpriteForwardPass>(
      "DebugSpriteForward", "DebugColor", "DebugColorRTV",
      m_activeWorld->draw_frame_state(),
      drawResources->sprite_instance_buffer_handle()));
  ...
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleSpriteForwardPass>(
          ...
          "EffectPreviewColorRTV", "EffectPreviewSceneDepth",
          "EffectPreviewSceneDepthDSV", m_activeWorld->particle_frame_state(),
          m_effectPreviewViewProjectionBufferHandle,
          particleResources->particle_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleTrailForwardPass>(
          "EffectPreviewParticleTrailForward", "EffectPreviewColor",
          "EffectPreviewColorRTV", "EffectPreviewSceneDepth",
          "EffectPreviewSceneDepthDSV", m_activeWorld->particle_frame_state(),
          particleResources->frame_buffer_handle(),
          m_effectPreviewViewProjectionBufferHandle,
  ...
  m_frameGraph->add_pass(
      std::make_unique<ParticleSystem::ParticleMeshForwardPass>(
          "EffectPreviewParticleMeshForward", "EffectPreviewColor",
          "EffectPreviewColorRTV", "EffectPreviewSceneDepth",
          "EffectPreviewSceneDepthDSV", m_activeWorld->particle_frame_state(),
          m_effectPreviewViewProjectionBufferHandle,
          particleResources->particle_buffer_handle()));
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectSpriteForwardPass>(
          "EffectPreviewEffectSpriteAlphaForward", "EffectPreviewColor",
          "EffectPreviewColorRTV", "EffectPreviewSceneDepth",
          "EffectPreviewSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_effectPreviewViewProjectionBufferHandle,
  ...
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectSpriteForwardPass>(
          "EffectPreviewEffectSpriteAdditiveForward", "EffectPreviewColor",
          "EffectPreviewColorRTV", "EffectPreviewSceneDepth",
          "EffectPreviewSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_effectPreviewViewProjectionBufferHandle,
  ...
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectRibbonForwardPass>(
          "EffectPreviewEffectRibbonAlphaForward", "EffectPreviewColor",
          "EffectPreviewColorRTV", "EffectPreviewSceneDepth",
          "EffectPreviewSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_effectPreviewViewProjectionBufferHandle,
  ...
  m_frameGraph->add_pass(
      std::make_unique<EffectSystem::EffectRibbonForwardPass>(
          "EffectPreviewEffectRibbonAdditiveForward", "EffectPreviewColor",
          "EffectPreviewColorRTV", "EffectPreviewSceneDepth",
          "EffectPreviewSceneDepthDSV",
          m_activeWorld->effect_primitive_frame_state(),
          m_effectPreviewViewProjectionBufferHandle,
          ...
          EffectSystem::EffectBlendMode::Additive));
  m_frameGraph->add_pass(
      std::make_unique<DrawSystem::DebugOutlinePass>(m_debugSelectedObjectId));
  m_frameGraph->add_pass(std::make_unique<DrawSystem::DebugPickReadbackPass>(
      m_debugPickState, m_debugPickReadbackBufferHandle,
...
std::function<void(uint64_t, uint32_t)> Engine::update() {
  return [this](uint64_t a_frameNo, uint32_t a_index) {
    ...
    if (is_playing() && m_scriptModuleHost != nullptr &&
        m_scriptModuleHost->runtime() != nullptr) {
      Result scriptResult = m_scriptModuleHost->runtime()->update(deltaTime);
      if (!scriptResult) {
    ...
    if (is_playing()) {
      ...
      if (m_scriptModuleHost != nullptr &&
        ...
        if (!collisionResult) {
          CUE_ASSERTF(false, "Script collision event dispatch failed: %s",
    ...
    Result updateResult = m_activeWorld->editor_update(
        a_index, m_backend->width(), m_backend->height(), editorDeltaTime);
    if (!updateResult) {
...
Result Engine::load_script_module(
  ...
  Result result = m_scriptModuleHost->load_module(a_scriptRoot, a_configuration,
                                                  *m_activeWorld);
  if (!result) {
    return result;
  }
...
Result Engine::start_play_mode() noexcept {
  if (m_editorWorld == nullptr || m_activeWorld == nullptr ||
      m_backend == nullptr || m_frameController == nullptr) {
    return Result::fail(Code::InvalidState, Severity::Error,
...
Result Engine::stop_play_mode() noexcept {
  if (m_editorWorld == nullptr || m_backend == nullptr ||
      m_frameController == nullptr) {
    return Result::fail(Code::InvalidState, Severity::Error,
                        "Engine play mode dependencies are not initialized.");
  }
