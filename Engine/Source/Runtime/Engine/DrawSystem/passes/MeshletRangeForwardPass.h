...
class MeshletRangeForwardPass final : public RHI::FrameGraphPass {
public:
  MeshletRangeForwardPass(const DrawFrameState &drawFrameState,
                          ...
                          uint32_t maxRangeDrawCommandCount)
      : m_drawFrameState(drawFrameState),
        ...
        m_maxRangeDrawCommandCount(maxRangeDrawCommandCount) {}

  const char *name() const noexcept override { return "MeshletRangeForward"; }
  ...
  Result setup(RHI::FrameGraphBuilder &builder) override {
    ...
    rootSignatureDesc.name = "MeshletRangeForwardRootSignature";
    ...
    for (uint32_t constantIndex = 3; constantIndex <= 10; ++constantIndex) {
      rootSignatureDesc.parameters.push_back(
          {RHI::RootParameterType::_32BitConstants, RHI::ShaderVisibility::All,
           constantIndex});
    }
    ...
    vsDesc.name = "MeshletRangeForwardVS";
    ...
    psDesc.name = "MeshletRangeForwardPS";
    ...
    pipelineDesc.name = "MeshletRangeForwardPipeline";
  ...
  Result describe_resources(RHI::FrameGraphBuilder &builder) override {
    ...
    result = builder.use_buffer(
        m_rangeDrawCommandBuffer, RHI::ResourceAccessType::Read,
        RHI::ResourceState::IndirectArgument, RHI::ResourceState::Common);
    if (!result) {
      return result;
    }
    result = builder.use_buffer(
        m_rangeDrawCountBuffer, RHI::ResourceAccessType::Read,
        RHI::ResourceState::IndirectArgument, RHI::ResourceState::Common);
    if (!result) {
      return result;
    }
  ...
  void execute(RHI::FrameGraphContext &context) override {
    ...
    commandContext->execute_indexed_indirect(m_rangeDrawCommandBuffer,
                                             m_rangeDrawCountBuffer,
                                             m_maxRangeDrawCommandCount);
  }
