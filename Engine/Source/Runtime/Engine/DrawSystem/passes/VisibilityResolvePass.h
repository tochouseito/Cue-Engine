...
class VisibilityResolvePass final : public RHI::FrameGraphPass {
public:
  VisibilityResolvePass(const RenderPath &renderPath,
      ...
      : m_renderPath(renderPath), m_debugView(debugView),
        ...
        m_pointLightBuffer(pointLightBuffer), m_enabled(enabled) {}

  const char *name() const noexcept override { return "VisibilityResolve"; }
  ...
  bool is_enabled(uint32_t a_frameIndex) const noexcept override {
    (void)a_frameIndex;
    return m_enabled && m_renderPath == RenderPath::VisibilityBuffer &&
  ...
  Result setup(RHI::FrameGraphBuilder &builder) override {
    ...
    rootSignatureDesc.name = "VisibilityResolveRootSignature";
    ...
    vsDesc.name = "VisibilityResolveVS";
    vsDesc.filePath = "Shaders/D3D12/VisibilityResolve.hlsl";
    ...
    psDesc.name = "VisibilityResolvePS";
    psDesc.filePath = "Shaders/D3D12/VisibilityResolve.hlsl";
    ...
    pipelineDesc.name = "VisibilityResolvePipeline";
  ...
  void execute(RHI::FrameGraphContext &context) override {
    ...
    commandContext->set_srv(5, m_meshRangeBuffer);
    commandContext->set_srv(6, m_transformBuffer);
    commandContext->set_cbv(8, m_lightFrameBuffer);
    commandContext->set_srv(9, m_directionalLightBuffer);
    commandContext->set_srv(10, m_pointLightBuffer);
    ...
    commandContext->set_32bit_constant(14, float_to_uint32(m_clusterNearZ));
    commandContext->set_32bit_constant(15, float_to_uint32(m_clusterFarZ));
    commandContext->set_32bit_constant(16,
                                       float_to_uint32(m_clusterInvLogFarNear));
    commandContext->set_srv(17, m_clusterLightRangeBuffer);
    commandContext->set_srv(18, m_clusterLightIndexBuffer);
    commandContext->set_srv(19, m_visibilityTriangleBuffer);
...
class VisibilityResolveComputePass final : public RHI::FrameGraphPass {
public:
  VisibilityResolveComputePass(const RenderPath &renderPath,
                               const RenderDebugView &debugView,
                               RHI::BufferHandle renderObjectBuffer,
                               RHI::BufferHandle transformBuffer,
                               RHI::BufferHandle viewProjectionBuffer,
  ...
  const char *name() const noexcept override {
    return "VisibilityResolveCompute";
  }
  ...
  Result setup(RHI::FrameGraphBuilder &builder) override {
    ...
    result = builder.get_view("FinalColorUAV", m_colorUav);
    if (!result) {
      return result;
    }
    result = builder.get_texture("VisibilityBuffer", m_visibility);
    ...
    RHI::RootSignatureDesc rootSignatureDesc{};
    rootSignatureDesc.name = "VisibilityResolveComputeRootSignature";
    rootSignatureDesc.parameters.push_back(
        {RHI::RootParameterType::DescriptorTableUAV,
    ...
    csDesc.name = "VisibilityResolveCS";
    csDesc.filePath = "Shaders/D3D12/VisibilityResolveCompute.hlsl";
    csDesc.entryPoint = "CSMain";
    csDesc.targetProfile = "cs_6_0";
    result = builder.create_shader_blob(csDesc, m_computeShader);
    if (!result) {
      return result;
    }

    RHI::ComputePipelineStateDesc pipelineDesc{};
    pipelineDesc.name = "VisibilityResolveComputePipeline";
    pipelineDesc.rootSignatureHandle = m_rootSignature;
  ...
  void execute(RHI::FrameGraphContext &context) override {
    ...
    commandContext->set_compute_descriptor_table(0, m_colorUav);
    commandContext->set_compute_descriptor_table(1, m_visibilitySrv);
    commandContext->set_cbv(2, m_viewProjectionBuffer);
    commandContext->set_32bit_constant(3, m_screenWidth);
    commandContext->set_32bit_constant(4, m_screenHeight);
    ...
    commandContext->set_srv(18, m_clusterLightIndexBuffer);
    commandContext->set_srv(19, m_visibilityTriangleBuffer);
    commandContext->dispatch((m_screenWidth + 7u) / 8u,
                             (m_screenHeight + 7u) / 8u, 1u);
  }
