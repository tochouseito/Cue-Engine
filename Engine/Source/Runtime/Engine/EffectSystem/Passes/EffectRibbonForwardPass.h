// EffectRibbonForwardPass の役割と公開要素を定義する
...
class EffectRibbonForwardPass final : public RHI::FrameGraphPass {
public:
  EffectRibbonForwardPass(std::string a_name, std::string a_colorName,
      ...
      : m_name(std::move(a_name)), m_colorName(std::move(a_colorName)),
        ...
        m_ribbonBufferHandle(a_ribbonBufferHandle), m_blendMode(a_blendMode) {}
  ...
  bool is_enabled(uint32_t a_frameIndex) const noexcept override {
    return a_frameIndex < m_frameState.frameStates.size() &&
           m_frameState.frame_state(a_frameIndex).frame.ribbonCount > 0;
  }
  ...
  Result setup(RHI::FrameGraphBuilder &builder) override {
    ...
    result = builder.read_buffer(m_ribbonBufferHandle);
    if (!result) {
      return result;
    }
    ...
    vertexShaderDesc.filePath = "Shaders/D3D12/EffectRibbonForward.hlsl";
    ...
    pixelShaderDesc.filePath = "Shaders/D3D12/EffectRibbonForward.hlsl";
  ...
  Result describe_resources(RHI::FrameGraphBuilder &builder) override {
    ...
    return builder.use_buffer(
        m_ribbonBufferHandle, RHI::ResourceAccessType::Read,
  ...
  void execute(RHI::FrameGraphContext &context) override {
    ...
    commandContext->set_srv(1, m_ribbonBufferHandle);
    commandContext->set_graphics_texture_table(2);
    commandContext->set_32bit_constant(3, static_cast<uint32_t>(m_blendMode));
    commandContext->draw_instanced(6 * GpuData::k_maxEffectRibbonSegmentCount,
                                   frameState.frame.ribbonCount, 0, 0);
  }
