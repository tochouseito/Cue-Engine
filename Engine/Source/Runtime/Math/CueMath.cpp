...
namespace Cue::Math
{
    ...
    constexpr uint32_t round_up_to_multiple(uint32_t value, uint32_t step) noexcept
    {
        ...
        if (step <= 0)
        {
            CUE_ASSERTF(false, "Invalid step: %u. Step must be greater than 0.", step);
        }
        ...
        if (value > (maxv - add))
        {
            CUE_ASSERTF(false, "Overflow detected: value %u + add %u exceeds max uint32_t %u.", value, add, maxv);
        }
