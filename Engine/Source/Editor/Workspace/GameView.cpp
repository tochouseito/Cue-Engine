...
{
    ImVec2 GameView::calculate_fit_size(ImVec2 a_availableRegion,
                                        uint32_t a_textureWidth,
                                        uint32_t a_textureHeight) noexcept
    {
        if (a_availableRegion.x <= 0.0f || a_availableRegion.y <= 0.0f ||
            a_textureWidth == 0 || a_textureHeight == 0)
        {
            return ImVec2(0.0f, 0.0f);
        }

        const float textureAspect =
            static_cast<float>(a_textureWidth) / static_cast<float>(a_textureHeight);
    ...
    void GameView::update()
    {
        ...
        const ImVec2 imageSize =
            calculate_fit_size(availableRegion, finalColorWidth, finalColorHeight);
