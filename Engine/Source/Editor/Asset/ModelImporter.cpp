...
void build_meshlet_ranges(Core::Native::MeshData &meshData) {
  ...
  std::vector<unsigned int> meshletVertices(maxMeshletCount *
                                            k_maxMeshletVertices);
  std::vector<unsigned char> meshletTriangles(maxMeshletCount *
                                              k_maxMeshletTriangles * 3u);

  const size_t meshletCount = meshopt_buildMeshlets(
      meshlets.data(), meshletVertices.data(), meshletTriangles.data(),
      meshData.indices.data(), meshData.indices.size(),
      &meshData.positions[0].x, meshData.positions.size(), sizeof(Math::float4),
      k_maxMeshletVertices, k_maxMeshletTriangles, k_coneWeight);
  ...
  for (const meshopt_Meshlet &meshlet : meshlets) {
    ...
    unsigned int *localVertices =
        meshletVertices.data() + meshlet.vertex_offset;
    unsigned char *localTriangles =
        meshletTriangles.data() + meshlet.triangle_offset;
    meshopt_optimizeMeshlet(localVertices, localTriangles,
                            meshlet.triangle_count, meshlet.vertex_count);

    const meshopt_Bounds bounds = meshopt_computeMeshletBounds(
        localVertices, localTriangles, meshlet.triangle_count,
    ...
    meshletBounds.coneApex = Math::float3(
        bounds.cone_apex[0], bounds.cone_apex[1], bounds.cone_apex[2]);
    meshletBounds.coneAxis = Math::float3(
        bounds.cone_axis[0], bounds.cone_axis[1], bounds.cone_axis[2]);
    meshletBounds.coneCutoff = bounds.cone_cutoff;
    const float coneAxisLengthSq =
        meshletBounds.coneAxis.dot(meshletBounds.coneAxis);
    const bool validCone = std::isfinite(meshletBounds.coneCutoff) &&
                           meshletBounds.coneCutoff >= -1.0f &&
                           meshletBounds.coneCutoff <= 1.0f &&
                           coneAxisLengthSq > 0.000001f;
    meshletBounds.flags = validCone ? 1u : 0u;
  ...
  if (meshData.rangeIndices.empty()) {
    meshData.rangeIndices = meshData.indices;
  }
}
...
Result
ModelImporter::import_model(const Core::IO::Path &filePath,
                            std::string_view modelName,
