...
Foundation配下のC++ SourceとHeaderは、PreprocessorのCRLF／LF行継続を論理行へ正規化し、Line／Block Commentを空白化し、垂直タブ／フォームフィードをSpaceへ、`%:`代替Tokenを`#`へ正規化してからIncludeとPragmaを検査します。Include検査ではHeader名の文字列を保持し、Pragma検査では通常文字列・文字・Raw String Literalを空白化したコードのみを使います。Literalな直接Includeは選択中Windows SDKの`um`、`shared`、`winrt`、`cppwinrt`、`ucrt`で解決し、Macro形式と絶対PathのIncludeは拒否します。Foundation内で解決できるProject Headerは未登録の拡張子も含めて再帰的に走査し、Foundation外で解決されるProject HeaderとFoundation外のTarget Include Directoryは拒否します。Source単位のInclude Directoryと`VS_SETTINGS`、Target単位の`VS_USER_PROPS`、`VS_PROJECT_IMPORT`、Unity Build挿入コードは空必須です。Header SetとTarget Sourceの全ファイルも拡張子に関係なく同じ内容検査へ含めます。Windows SDKの前4 DirectoryのHeaderと、UCRT内のISO C標準Header以外を拒否するため、限定したPlatform Header名の列挙には依存しません。PCHと公開Compile Optionを空必須とし、非公開Compile OptionとFoundation Target定義ScopeでDebug／Development／Releaseへ適用されるGlobal C++ Flagは現在必要な警告、準拠、UTF-8、Runtime既定、最適化・Debug情報のAllowlistだけを許可します。Targetの`COMPILE_FLAGS`とSource単位の`COMPILE_FLAGS`／`COMPILE_OPTIONS`も空必須とし、`SOURCES`とHeader Set内のGenerator Expression、およびFoundation外のHeader Set Fileを禁止します。加えて、全走査対象Source／Header上の`#pragma comment(...)`と、単一行・複数行を問わず`__pragma(...)`／`_Pragma(...)`を拒否し、生成された`Cue.Foundation.lib`のLinker Directiveも`dumpbin`で検査します。`/`と`-`のOption Prefixを正規化し、MSVC Runtime以外の`DEFAULTLIB`と未承認Directiveを拒否します。
...
- Negative Injectionとして`INTERFACE_LINK_OPTIONS`の`/DEFAULTLIB:d3d12.lib`、`SOURCES`の`$<TARGET_OBJECTS:...>`、Custom Commandの`$<TARGET_FILE:...>`、生成SourceのCustom Target依存、`processthreadsapi.h`と`io.h`の直接Include、Macro経由、絶対Path、Comment分割、`%:include`、垂直タブ／フォームフィード空白形式の`windows.h` Include、Foundation外のProject Headerへの相対Include、未登録のFoundation内`.ipp`を介した`windows.h` Include、Foundation外のTarget Include DirectoryとSource単位Include Directory、`VS_USER_PROPS`、`VS_PROJECT_IMPORT`、Source単位`VS_SETTINGS`、Unity BuildのInclude前後挿入コード、公開PCHの`windows.h`、非公開・条件Generator Expression内・`SHELL:`内・分割Generator Expression内・RootとTarget定義ScopeのGlobal Debug Flagにある`/FIwindows.h`、TargetとSource単位の`COMPILE_FLAGS`による`/FIwindows.h`、条件付きSource Generator ExpressionとSource単位`/FIwindows.h`の組み合わせ、公開Header Setの条件Generator Expression、Header Set内`.ipp`とprivate Target Source `.hxx`からの`windows.h` Include、単一行、行継続、Comment分割形式の`#pragma comment(lib, "d3d12.lib")`、実装Sourceと未使用公開Header内の単一行・複数行`__pragma(comment(lib, "d3d12.lib"))`、`_Pragma`、`__pragma(comment(linker, "-defaultlib:d3d12.lib"))`がそれぞれGateを失敗させることを確認。通常文字列Literal内のCompiler Pragma文字列とRaw String内のDirective文字列は誤検出しないことも確認
- `git diff --check`成功
