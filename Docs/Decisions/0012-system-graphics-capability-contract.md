...
ADR-0007に従い、`GraphicsBackend::capabilities()`の呼び出しはBackend生成Threadに限定し、このAccessor呼び出し時のThread違反だけを既存RHI Assertで検出する。戻り値は公開Fieldを持つ値型への`const`参照であり、Field読取時にThread検査を行わない。

取得したBackend所有参照を別Threadへ転送することは呼び出し側の前提違反とし、読取時Assertを保証しない。Worker Threadで使用する場合はBackend生成Thread上でSnapshotをCopyし、Backend所有参照ではなく独立値をMoveまたはCopyする。独立CopyのReadにはRHI Thread Affinityを適用しない。
